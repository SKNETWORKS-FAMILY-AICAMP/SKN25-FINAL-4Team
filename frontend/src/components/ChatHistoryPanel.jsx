...
const MD_COMPONENTS = {
  h1: ({ children }) => <div style={{ fontSize: 15, fontWeight: 700, color: '#1b2433', marginTop: 10, marginBottom: 6 }}>{children}</div>,
  h2: ({ children }) => <div style={{ fontSize: 14, fontWeight: 700, color: '#1b2433', marginTop: 10, marginBottom: 6 }}>{children}</div>,
  h3: ({ children }) => <div style={{ fontSize: 13, fontWeight: 700, color: '#2563eb', marginTop: 8, marginBottom: 4 }}>{children}</div>,
  p:  ({ children }) => <p style={{ margin: '0 0 8px', lineHeight: 1.65, color: '#1b2433' }}>{children}</p>,
  ul: ({ children }) => <ul style={{ margin: '4px 0 8px', paddingLeft: 18, listStylePosition: 'outside', color: '#1b2433' }}>{children}</ul>,
  ol: ({ children }) => <ol style={{ margin: '4px 0 8px', paddingLeft: 18, listStylePosition: 'outside', color: '#1b2433' }}>{children}</ol>,
  li: ({ children }) => <li style={{ marginBottom: 3, lineHeight: 1.6, color: '#1b2433' }}>{children}</li>,
  ...
  hr: () => <hr style={{ border: 'none', borderTop: '1px solid #e2e7ef', margin: '10px 0' }} />,
  blockquote: ({ children }) => (
    <blockquote style={{ borderLeft: '3px solid #2563eb', paddingLeft: 10, margin: '6px 0', color: '#5a6675', fontStyle: 'italic' }}>
      {children}
    </blockquote>
  ),
  code: ({ inline, children }) => inline
    ? <code style={{ background: '#f4f6fa', border: '1px solid #e2e7ef', borderRadius: 4, padding: '1px 5px', fontSize: 11, color: '#0550ae', fontFamily: 'monospace' }}>{children}</code>
    : <pre style={{ background: '#f4f6fa', border: '1px solid #e7ebf1', borderRadius: 6, padding: '10px 12px', overflowX: 'auto', marginBottom: 8 }}>
        <code style={{ fontSize: 11, color: '#1b2433', fontFamily: 'monospace', whiteSpace: 'pre' }}>{children}</code>
      </pre>,
}
    ...
    <div style={s.wrap}>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, color: '#909aa8' }}>{sessions.length}개 세션</span>
          <button
          ...
          <div style={s.sessionList}>
            ...
            {!loading && sessions.length === 0 && (
              <div style={s.empty}>
                ...
                <span style={{ fontSize: 11, color: '#909aa8' }}>
                  우하단 💬 챗봇으로 대화를 시작하세요.
                </span>
              </div>
            )}
                ...
                <div style={s.sessionMeta}>
                  ...
                  {sess.last_intent && (
                    <span style={{
                      ...
                      color:      INTENT_COLOR[sess.last_intent] ?? '#909aa8',
                      fontWeight: 600,
                    }}>
        ...
        <div style={s.viewer}>
          {!selectedId && (
            <div style={s.viewerEmpty}>
              ...
              <div style={{ fontSize: 14, color: '#1b2433', marginBottom: 4 }}>
                좌측 목록에서 대화를 선택하세요
              </div>
                ...
                {(detail.messages ?? []).map((m, i) => (
                  <div key={i} style={{ ...s.row, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    ...
                    <div style={{ ...s.bubble, ...(m.role === 'user' ? s.userBubble : s.aiBubble) }}>
                      {m.intent && (
                        <span style={{ ...s.badge, background: INTENT_COLOR[m.intent] ?? '#909aa8' }}>
...
const s = {
  ...
  clearAllBtn: { padding: '5px 10px', background: '#e7ebf1', border: '1px solid #f8514944', borderRadius: 6, color: '#f85149', fontSize: 11, cursor: 'pointer', fontWeight: 600 },

  body:    { flex: 1, display: 'grid', gridTemplateColumns: '240px 1fr', overflow: 'hidden' },

  // 좌측 세션 리스트
  sidebar:     { borderRight: '1px solid #e7ebf1', display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  search:      { margin: 12, padding: '8px 12px', background: '#f4f6fa', border: '1px solid #e2e7ef', color: '#1b2433', borderRadius: 6, fontSize: 12, outline: 'none' },
  sessionList: { flex: 1, overflowY: 'auto', padding: '0 8px 8px' },
  empty:       { padding: 24, textAlign: 'center', color: '#5a6675', fontSize: 12, lineHeight: 1.7 },
  ...
  viewerEmpty:  { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#5a6675', fontSize: 13, padding: 40, textAlign: 'center' },
  viewerHeader: { padding: '12px 20px', borderBottom: '1px solid #e7ebf1', background: '#ffffff' },
  viewerTitle:  { fontSize: 14, fontWeight: 600, color: '#1b2433' },
  viewerMeta:   { fontSize: 11, color: '#5a6675', marginTop: 3 },
  ...
  avatar:      { width: 28, height: 28, borderRadius: '50%', background: '#e7ebf1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0, marginTop: 2 },
  ...
  aiBubble:    { background: '#ffffff', border: '1px solid #e2e7ef', borderBottomLeftRadius: 4, color: '#1b2433' },
  badge:       { display: 'inline-block', fontSize: 10, padding: '1px 7px', borderRadius: 4, color: '#f4f6fa', fontWeight: 700, marginBottom: 6 },
  mdWrap:      { lineHeight: 1.65, fontSize: 13, overflowWrap: 'anywhere' },
  ts:          { fontSize: 9, color: '#909aa8', marginTop: 6, textAlign: 'right' },
}
