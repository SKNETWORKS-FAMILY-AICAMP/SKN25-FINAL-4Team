...
export default function BillingPanel() {
  ...
  const statusColor = data ? (STATUS_COLOR[data.status] ?? '#5a6675') : '#5a6675'
    ...
    <div style={s.wrap}>
      ...
      {!loading && data && (
        <div style={s.body}>
          ...
          {/* 상단 KPI 카드 4개 */}
          <div style={s.kpiRow}>
            <KpiCard label="이번 달 목표 요금" value={fmtEur(data.target_eur)} sub={fmtKrw(data.target_eur, data.krw_rate)} color="#1b2433"/>
...
function KpiCard({ label, value, sub, color, highlight }) {
  return (
    <div style={{ ...s.kpiCard, ...(highlight ? s.kpiHighlight : {}) }}>
      <div style={{ fontSize: 11, color: '#5a6675', marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 700, color }}>{value}</div>
      <div style={{ fontSize: 10, color: '#909aa8', marginTop: 4 }}>{sub}</div>
    </div>
  )
}
...
const s = {
  ...
  input:       { width: 64, padding: '4px 8px', background: '#f4f6fa', border: '1px solid #e2e7ef', color: '#1b2433', borderRadius: 4, fontSize: 12, textAlign: 'right' },
  ...
  loading:     { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#5a6675' },
  ...
  kpiCard:     { background: '#ffffff', border: '1px solid #e7ebf1', borderRadius: 10, padding: '14px 18px' },
  kpiHighlight:{ background: 'linear-gradient(135deg, #2563eb14, #ffffff)', border: '1px solid #2563eb55', boxShadow: '0 0 16px rgba(31, 111, 235, 0.1)' },

  narrativeBox:  { background: 'linear-gradient(135deg, #a371f70a, #ffffff)', border: '1px solid #a371f733', borderRadius: 10, padding: '14px 18px' },
  narrativeTitle:{ fontSize: 12, fontWeight: 700, color: '#a371f7', marginBottom: 6 },
  narrativeText: { fontSize: 13, color: '#33404f', lineHeight: 1.7, whiteSpace: 'pre-wrap' },

  chartsRow:   { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 },
  chartBox:    { background: '#ffffff', border: '1px solid #e7ebf1', borderRadius: 10, padding: '12px 14px' },
  chartTitle:  { fontSize: 12, fontWeight: 600, color: '#1b2433', marginBottom: 8 },

  infoBox:     { padding: '10px 14px', background: '#ffffff', border: '1px solid #e7ebf1', borderRadius: 8, textAlign: 'center' },
}
