...
export default function chat_workspace_panel() {
  const [sessions, setSessions] = useState([])
  const [search,   setSearch]   = useState('')
  const [loading,  setLoading]  = useState(true)

  // 우측 채팅 제어
  const [chatKey,        setChatKey]        = useState('new-0')   // 변경 시 chat_panel 리마운트
  ...
  // chat_panel이 세션을 만들면(또는 메시지 추가) 목록만 갱신 — 리마운트는 안 함
