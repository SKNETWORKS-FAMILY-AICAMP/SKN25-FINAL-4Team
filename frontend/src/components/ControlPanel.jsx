...
const STATUS_LABEL = {
  pending:   { label: '검토 대기', color: '#5a6675', bg: '#e7ebf1' },
  approved:  { label: '✓ 승인 완료', color: '#3fb950', bg: '#3fb95022' },
  rejected:  { label: '✕ 거부됨', color: '#909aa8', bg: '#ffffff' },
}
      ...
      <div style={s.body}>
        {loading && (
          <div style={s.center}>
            ...
            <div style={{ fontSize: 13, color: '#5a6675' }}>예측·이상·효율 데이터 종합 분석 중...</div>
          </div>
        )}
        ...
        {!loading && !error && items.length === 0 && (
          <div style={s.center}>
            ...
            <div style={{ fontSize: 14, color: '#1b2433', fontWeight: 600 }}>현재 활성 권고 없음</div>
...
function RecCard({ item, busy, onAction }) {
  ...
  const memColor = mem?.label === '성공' ? '#3fb950' : mem?.label === '실패' ? '#f85149' : '#5a6675'
  const learned = item.learned   // {success, failure, rate, signal} or undefined
  const lrColor = learned?.signal === 'trusted' ? '#3fb950' : learned?.signal === 'caution' ? '#f85149' : '#5a6675'
  ...
  return (
    <div style={{ ...s.reviewedItem, display: 'block', cursor: 'pointer', padding: 0 }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, color: '#1b2433', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {item.title}
          </div>
          <div style={{ fontSize: 10, color: '#909aa8', marginTop: 2 }}>
        ...
        <span style={{ fontSize: 11, color: '#909aa8', width: 12, textAlign: 'center', flexShrink: 0 }}>
          {open ? '▾' : '▸'}
        </span>
      </div>
      {open && (
        <div style={s.reviewedDetail} onClick={e => e.stopPropagation()}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 10, color: '#5a6675' }}>카테고리: {CATEGORY_LABEL[item.category] ?? item.category}</span>
          </div>
          <div style={{ fontSize: 12, color: '#33404f', lineHeight: 1.6, marginBottom: 8 }}>
            {item.description}
          </div>
          <div style={{ display: 'flex', gap: 16, paddingTop: 8, borderTop: '1px solid #e7ebf1' }}>
            <div>
              <div style={{ fontSize: 9, color: '#909aa8', textTransform: 'uppercase' }}>대상 설비</div>
              <div style={{ fontSize: 12, color: '#1b2433', fontWeight: 500, marginTop: 2 }}>{item.equipment}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, color: '#909aa8', textTransform: 'uppercase' }}>예상 효과</div>
...
const s = {
  ...
  header:      { padding: '14px 24px', borderBottom: '1px solid #e7ebf1', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: '#ffffff', gap: 16, flexWrap: 'wrap' },
  ...
  refreshBtn:  { padding: '6px 14px', background: '#e7ebf1', border: '1px solid #e2e7ef', borderRadius: 6, color: '#1b2433', fontSize: 12, fontWeight: 500, cursor: 'pointer' },
  clearBtn:    { padding: '6px 14px', background: '#e7ebf1', border: '1px solid #f8514944', borderRadius: 6, color: '#f85149', fontSize: 12, fontWeight: 600, cursor: 'pointer' },
  ...
  sectionTitle:{ fontSize: 12, fontWeight: 700, color: '#5a6675', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  cardGrid:    { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(420px, 1fr))', gap: 14 },

  card:        { background: '#ffffff', border: '1px solid #e2e7ef', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 },
  cardHeader:  { display: 'flex', alignItems: 'flex-start', gap: 10 },
  cardCat:     { fontSize: 10, color: '#5a6675', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 },
  cardTitle:   { fontSize: 13, fontWeight: 600, color: '#1b2433', lineHeight: 1.4 },
  priBadge:    { fontSize: 10, padding: '3px 8px', borderRadius: 4, fontWeight: 700, flexShrink: 0 },
  cardDesc:    { fontSize: 12, color: '#33404f', lineHeight: 1.6 },
  cardMeta:    { display: 'flex', gap: 16, padding: '8px 0', borderTop: '1px solid #e7ebf1', borderBottom: '1px solid #e7ebf1' },
  metaItem:    { display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 0 },
  metaLabel:   { fontSize: 9, color: '#909aa8', textTransform: 'uppercase', letterSpacing: 0.5 },
  metaValue:   { fontSize: 12, color: '#1b2433', fontWeight: 500 },
  cardActions: { display: 'flex', gap: 8, justifyContent: 'flex-end' },
  btnGhost:    { padding: '7px 14px', background: 'transparent', border: '1px solid #e2e7ef', borderRadius: 6, color: '#5a6675', fontSize: 12, fontWeight: 500, cursor: 'pointer' },
  ...
  reviewedItem:  { display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#f4f6fa', border: '1px solid #e7ebf1', borderRadius: 6 },
  reviewedDetail:{ padding: '10px 14px 12px', borderTop: '1px solid #e7ebf1', background: '#0a0d11', borderRadius: '0 0 6px 6px' },
}
