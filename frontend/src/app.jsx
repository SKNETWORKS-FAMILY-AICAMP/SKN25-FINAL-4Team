...
import dashboard_panel    from './components/panels/dashboard_panel'
import chat_panel         from './components/panels/chat_panel'
import chat_workspace_panel from './components/panels/chat_workspace_panel'
import simulator_clock    from './components/common/simulator_clock'
import anomaly_panel      from './components/panels/anomaly_panel'
import equipment_panel    from './components/panels/equipment_panel'
import maintenance_panel  from './components/panels/maintenance_panel'
import report_panel     from './components/panels/report_panel'
import forecast_panel   from './components/panels/forecast_panel'
import topology_panel   from './components/panels/topology_panel'
import settings_panel   from './components/panels/settings_panel'
import users_panel      from './components/panels/users_panel'
import control_panel    from './components/panels/control_panel'
import billing_panel    from './components/panels/billing_panel'
import login_screen     from './components/login_screen'
import { BASE, getBackendHealth, getMe, logout as apiLogout, setAuthToken, getToken } from './api/client'
import {
...
const TABS = [
  ...
  { id: 'topology',    label: '계량기 토폴로지', icon: Network },
...
const PANELS = {
  ...
  chat:      chat_workspace_panel,
  anomaly:   anomaly_panel,
  report:    report_panel,
...
export default function app() {
  ...
  useEffect(() => {
    const check = () =>
      getBackendHealth()
        .then(r => setApiOk(r.status === 200 && r.data?.status === 'ok'))
  ...
  if (!user) return <login_screen onLogin={setUser} />
    ...
    <div style={styles.app}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={styles.simBar}>
          <div style={{ fontSize: 10, color: T.textFaint, fontWeight: 700, letterSpacing: 0.8 }}>
            REAL-TIME SIMULATION
          </div>
          <simulator_clock onAnomalyClick={() => handleTab('anomaly')} />
        </div>
          ...
          <main style={styles.main}>
            {TABS.map(({ id }) => {
              ...
              const extraProps =
                ...
                id === 'maintenance' ? { refreshSignal: woVersion } :
              ...
              return (
                <div key={id} style={{ display: tab === id ? 'flex' : 'none', flexDirection: 'column', height: '100%' }}>
                  {Panel ? <Panel {...extraProps} /> : <div style={{ padding: 20, color: '#8b949e' }}>Component not found</div>}
                </div>
              )
            })}
          </main>
...
const TOAST_LEVEL_STYLE = {
  HIGH:     { border: '#f85149', icon: '🚨', bg: '#2d1517' },
  CRITICAL: { border: '#f85149', icon: '🚨', bg: '#2d1517' },
  WARNING:  { border: '#d29922', icon: '⚠️',  bg: '#2b2409' },
  INFO:     { border: '#1f6feb', icon: 'ℹ️',  bg: '#0d1d36' },
}
