...
┌──────────────────────────────────────────────────────────────────┐
│  사용자 브라우저                                                    │
│  React (Vite) — 12개 패널 대시보드                                 │
└────────────────────────┬─────────────────────────────────────────┘
                         │ HTTP / SSE (Server-Sent Events)
┌────────────────────────▼─────────────────────────────────────────┐
│  FastAPI (Python 3.13)                                            │
│  ├── LangGraph 멀티에이전트 (5개 에이전트 + Critic)                │
│  │   └── LLM 호출 → Ollama (RunPod RTX 3090)                     │
│  │        ├── EXAONE 3.5:7.8B  ← fast 모드 (의도 분류)           │
│  │        └── Gemma4:12B       ← quality 모드 (진단·보고서)        │
│  ├── ML 예측 (v84 앙상블 — LSTM×6 + CatBoost + LightGBM + Ridge) │
│  ├── ML 이상탐지 (residual + IsolationForest / LSTM-AE 투표)      │
...
- `/cms`, `/anomalies`, `/report`, `/forecast`, `/control`, `/settings` 등 13개 라우터
...
**이 프로젝트에서의 역할:**
- 매월 1일 자정 v84 앙상블 자동 재학습 (`ml.pipeline.train` 서브프로세스 실행)
- 매일 오전 6시 일일 보고서 자동 생성
- 설정 패널에서 스케줄 ON/OFF, 시간 변경 가능
...
[classify 노드]  ← 키워드 룰 우선, 모호하면 LLM(EXAONE fast) 폴백
    ...
    ├─ cms      → [cms_agent]       → 설비 진단 + LLM 원인 분석
    ├─ rag      → [rag_agent]       → pgvector 검색 + LLM 답변
    └─ off_topic → [rejection_node] → LLM 호출 없이 거절 (즉시 반환)
    │
    ▼
[critic 노드]  ← 에너지 도메인 용어 교정 (한전→독일 공공 전력망 등 8개 패턴)
    │
    ▼
최종 답변
```
...
**의도 분류 상세:**
- **1차 방어**: 23개 정규식 키워드 룰 (LLM 호출 없음, <1ms)
- **2차 방어**: LLM 폴백 — EXAONE fast 모드, `max_tokens=10`, ~6s
- **오프토픽 차단**: `_KW_OFFTOPIC` 패턴(주식·요리·날씨 등) → 즉시 거절
...
| 구분 | 모델 | 역할 | 응답속도 |
|------|------|------|----------|
| **fast 모드** | EXAONE 3.5:7.8B (LG AI Research) | 의도 분류 (`max_tokens=10`) | ~6초 |
| **quality 모드** | Gemma4:12B (Google DeepMind) | 진단·보고서·RAG·이상탐지 분석 | ~20초 (thinking 활성화) |

**모델 선정 근거 (33문항 GPT-4o-as-Judge 평가, 5축 10점 척도):**

| 모델 | 종합 | 한국어 | 형식 | 근거성 | 논리성 | 실용성 | 속도 | 자체 호스팅 |
|------|------|--------|------|--------|--------|--------|------|-------------|
| **Gemma4 12B** | **9.2** | 9.2 | 9.3 | 8.9 | 9.4 | 9.2 | ~20s | ✅ |
| GPT-4o | 9.1 | 9.1 | 9.0 | 8.8 | 9.4 | 9.3 | 3.4s | ❌ 클라우드 |
| EXAONE 3.5 7.8B | 8.9 | 8.9 | 8.6 | 8.8 | 9.0 | 9.0 | 6.0s | ✅ |
| Qwen2.5 7B | 2.4 | 3.9 | 3.8 | 4.5 | 0.0 | 0.0 | 4.8s | ✅ (탈락) |

> Qwen2.5 탈락 이유: 복잡한 수치 추론 시 중국어로 전환 — 논리성·실용성 0점.
...
def chat(messages, max_tokens=1024, fast=False):
    ...
    # fast=True  → EXAONE 3.5:7.8B (think=false)
    # fast=False → Gemma4:12B (thinking 자동 활성화)
```
...
| 항목 | 내용 |
|------|------|
| **openai** `>=1.0.0` | GPT-4o 등 OpenAI 모델. 개발/테스트 시 기준선 |
...
**앙상블 방식:** v63·v67·v71 세 버전의 LSTM 예측을 **중앙값(median)**으로 앙상블 → 검증 데이터 기반 시간대별 shrunk bias correction × gain 1.30 후처리.
...
**이 프로젝트에서의 역할:**
- LSTM이 잡지 못하는 비선형 패턴 포착
- 학습 파일: `catboost.cbm` (HF Hub에 보관)
...
| 패널 | 역할 |
|------|------|
| DashboardPanel | KPI 요약 + 실시간 시뮬레이터 |
| EquipmentPanel | 설비별 헬스스코어 + AI 진단 |
| MaintenancePanel | 작업지시 칸반 |
| AnomalyPanel | 이상탐지 이력 + 차트 |
| ChatPanel / ChatWorkspacePanel | AI 대화 + 세션 관리 |
| ForecastPanel | 수요 예측 시각화 |
| ControlPanel | 운영 권고 + 적응형 학습 |
| BillingPanel | 목표 요금 관리 |
| ReportPanel | 월간/일일 보고서 (AI 생성 포함) |
| TopologyPanel | 계량기 에너지 흐름 토폴로지 |
| SettingsPanel | LLM 설정 (프로바이더/모델/API키) |
| UsersPanel | 사용자 관리 |
...
`frontend/src/api/client.js`: 모든 백엔드 API 호출 집중 관리.
...
| 항목 | 내용 |
|------|------|
| **react-markdown** | Markdown 텍스트를 React 컴포넌트로 렌더링 |
| **katex** | 수식($E=mc^2$) 렌더링 |

AI 진단 결과, 보고서 텍스트를 Markdown으로 받아 렌더링.
...
| 항목 | 내용 |
|------|------|
| **무엇** | 비동기 HTTP 클라이언트. FastAPI 테스트, 외부 HTTP 호출(Ollama 직접 API 등)에 사용. |
| **버전** | `>=0.27.0` |

`llm_client.py`에서 Ollama 네이티브 `/api/chat` 엔드포인트 직접 호출 시 사용 (OpenAI 호환 우회 필요 시).
...
| pgvector | 최신 | 벡터 검색 |
| Ollama | v0.30.3+ | LLM 로컬 서빙 |
| Gemma4:12B | Google DeepMind | quality 모드 LLM |
