...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "import_pmax_outputs"
...
<body>
  ...
  <p>이 모델은 <code>mart.peak_feature_15min</code>에서 pivot한 <code>P.max_value</code>를 0 이상으로 보정한 import P_max를 타깃으로 사용해 다음 {html.escape(output_range)} 수전 피크 전력값을 예측합니다. 입력은 과거 {input_hours}시간의 15분 feature입니다.</p>
...
<body>
  ...
  <p>이 실험은 개별 logical meter의 다음 15분 import 피크 전력값, 즉 <code>max(P.max_value, 0)</code>를 예측합니다. Persistence baseline은 직전 15분 import P_max 반복값입니다.</p>
  <h2>전체 결과</h2>
...
def main() -> None:
    ...
    output_dir = args.output_dir or (
        SCRIPT_DIR / ("import_pmax_outputs_gpu" if args.device == "gpu" else "import_pmax_outputs")
    )
    output_dir.mkdir(parents=True, exist_ok=True)
