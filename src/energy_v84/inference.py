...
CLI:
  conda run -n skn25 python -m energy_v84.inference \\
...
METER_TAGS_PATH      = Path(__file__).resolve().parent / "common" / "meter_tags.csv"
...
def _failed_row(meter_urn: str, model_urn: str, horizon: int,
                timestamp: pd.Timestamp, status: str) -> dict:
    row: dict = {
        "meter_urn":     meter_urn,
        "model_urn":     model_urn,
        ...
        "warning_flag":                False,
        "warning_reason_code":         "NO_PREDICTION",
...
from energy_v84.common.db import build_engine, fetch_meter_window
from energy_v84.common.preprocessing import (
...
from energy_v84.common.model import RecurrentPredictor, device, predict_scaled
from energy_v84.common.catboost_model import predict_catboost_scaled
from energy_v84.common.ridge import predict_ridge_scaled
...
def _compute_input_quality(raw: pd.DataFrame, spec: MeterSpec,
    ...
    # ── input_missing_count ──────────────────────────────────────────────────
    # 아예 없는 ts(gap) + spec.features 중 하나라도 NaN인 행
    gap_count = sum(1 for t in full_idx if t not in existing_ts)
    feat_cols = [c for c in spec.features if c in window_raw.columns]
    if feat_cols:
        feat_nan_mask = window_raw[feat_cols].apply(
    ...
    # spec.features 전체 기준: reindex → 물리 위반 NaN 처리 → ffill 전후 비교
    ...
    feat_nan_any = feat_nan_mask if feat_cols else pd.Series(False, index=window_raw.index)
...
def run_inference(
    ...
    for meter_urn, info in sorted(METER_MAP.items()):
        ...
        if not art_path.exists():
            ...
            failed = _failed_row(meter_urn, model_urn, horizon, timestamp, "no_artifact")
            rows.append(failed)
        ...
        try:
            ...
            if res is None:
                ...
                failed = _failed_row(meter_urn, model_urn, horizon, timestamp, "insufficient_data")
                rows.append(failed)
                results[meter_urn] = failed
                continue
            ...
            failed = _failed_row(meter_urn, model_urn, horizon, timestamp, "error")
            rows.append(failed)
            results[meter_urn] = failed
...
def _parse_args():
    p = argparse.ArgumentParser(description="energy_v84 inference pipeline")
...
def main():
    ...
    print(f"=== energy_v84 inference | horizon={args.horizon}h | ts={ts} ===")
