...
PMAX_REFERENCE_FEATURE_TABLE = "reference.corrected_resampled_15min"
REFERENCE_WARM_START_EXCLUSIVE_END_TS = "2023-12-01T00:00:00+00:00"
...
def build_pmax_feature_query(
    *,
    ...
    if source_mode == SOURCE_MODE_REFERENCE_BACKFILL:
        ...
        source_mode_clause = "(source_mode = %(live_source_mode)s OR source_mode = %(reference_source_mode)s)"
        source_preference_order = (SOURCE_MODE_LIVE_OBSERVED, SOURCE_MODE_REFERENCE_BACKFILL)
    order_terms = "created_at DESC NULLS LAST, run_id DESC NULLS LAST"
    if source_mode in {SOURCE_MODE_REFERENCE_BACKFILL, SOURCE_MODE_HYBRID_WARM_START}:
        source_rank = "CASE WHEN source_mode = %(live_source_mode)s THEN 0 WHEN source_mode = %(reference_source_mode)s THEN 1 ELSE 2 END"
        order_terms = f"{source_rank}, {order_terms}"
    if source_mode == SOURCE_MODE_HYBRID_WARM_START:
        sql = f"""
WITH hybrid_warm_start_peak_feature_adapter AS (
  SELECT {columns}
  FROM {input_table}
  WHERE window_ts BETWEEN %(input_start_ts)s AND %(input_end_ts)s
    ...
    AND (source_mode = %(live_source_mode)s OR source_mode = %(reference_source_mode)s)
  UNION ALL
  SELECT
    ts AS window_ts,
    meter_urn,
    ...
    value AS p95_value,
    value AS p99_value,
    0.0::double precision AS std_value,
    value AS last_value,
    ts AS peak_ts,
    value AS peak_value,
    15::integer AS observed_points,
    15::integer AS expected_points,
    1.0::double precision AS coverage_ratio,
    source_file,
    %(pmax_reference_table)s::text AS source_layer,
    %(reference_source_mode)s::text AS source_mode,
    jsonb_build_object(
      'adapter', 'hybrid_warm_start_peak_feature_adapter',
      'source_table', %(pmax_reference_table)s::text,
      'source_mode', %(reference_source_mode)s::text,
      'exclusive_end_ts', %(reference_warm_start_exclusive_end_ts)s::text
    ) AS provenance,
    run_id,
    created_at
  FROM {PMAX_REFERENCE_FEATURE_TABLE}
  WHERE ts BETWEEN %(input_start_ts)s AND %(input_end_ts)s
    AND ts < %(reference_warm_start_exclusive_end_ts)s::timestamptz
    AND meter_urn IN ({placeholders_sources})
    AND measurement IN ({placeholders_measurements})
), ranked AS (
  SELECT *,
         row_number() OVER (
           ...
           ORDER BY CASE
             WHEN source_mode = %(live_source_mode)s THEN 0
             WHEN source_mode = %(reference_source_mode)s THEN 1
             ELSE 2
           END,
           created_at DESC NULLS LAST,
           run_id DESC NULLS LAST
         ) AS source_rank
  FROM hybrid_warm_start_peak_feature_adapter
)
SELECT {columns}
FROM ranked
WHERE source_rank = 1
ORDER BY window_ts, meter_urn, measurement
""".strip()
    else:
        sql = f"""
SELECT {columns}
...
""".strip()
    params: dict[str, Any] = {
        ...
        "pmax_reference_table": PMAX_REFERENCE_FEATURE_TABLE,
        "reference_warm_start_exclusive_end_ts": REFERENCE_WARM_START_EXCLUSIVE_END_TS,
    }
    ...
    return SqlQuerySpec(
        ...
        sql=sql,
        params=params,
        source_tables=(input_table, PMAX_REFERENCE_FEATURE_TABLE) if source_mode == SOURCE_MODE_HYBRID_WARM_START else (input_table,),
        expected_columns=PMAX_FEATURE_QUERY_COLUMNS,
        source_contract={
            "source_mode": source_mode,
            "allowed_source_modes": source_preference_order,
            "allow_null_source_mode": source_mode == SOURCE_MODE_REFERENCE_BACKFILL,
            ...
            "reference_warm_start_exclusive_end_ts": REFERENCE_WARM_START_EXCLUSIVE_END_TS if source_mode == SOURCE_MODE_HYBRID_WARM_START else None,
        },
    )
...
__all__ = [
    ...
    "PMAX_FEATURE_QUERY_COLUMNS",
    "PMAX_REFERENCE_FEATURE_TABLE",
    "REFERENCE_WARM_START_EXCLUSIVE_END_TS",
    "SCHEMA_INVENTORY_TABLES",
    "SqlQuerySpec",
