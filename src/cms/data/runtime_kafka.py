...
def make_kafka_consumer_config(env: dict[str, str] | None = None) -> dict[str, object]:
    ...
    optional_string_settings = {
        "group.instance.id": values.get("KAFKA_CONSUMER_INSTANCE_ID") or values.get("KAFKA_GROUP_INSTANCE_ID"),
        "partition.assignment.strategy": values.get("KAFKA_PARTITION_ASSIGNMENT_STRATEGY"),
    }
    for key, value in optional_string_settings.items():
        if value:
            config[key] = value
    optional_integer_settings = {
        "session.timeout.ms": values.get("KAFKA_SESSION_TIMEOUT_MS"),
        "max.poll.interval.ms": values.get("KAFKA_MAX_POLL_INTERVAL_MS"),
    }
    for key, value in optional_integer_settings.items():
        if value:
