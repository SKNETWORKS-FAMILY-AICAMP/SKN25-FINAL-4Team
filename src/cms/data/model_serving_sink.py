...
MODEL_SERVING_EVIDENCE_TABLE = "qa.model_serving_evidence_packet"
    ...
    *,
    ...
    batches = (
        ...
        ModelServingTableBatch(
            PMAX_FORECAST_INFERENCE_LOG_TABLE,
            (_pmax_log_payload(run_id, job_id, _pmax_base_ts(forecast_rows, started_at), started_at, finished_at, pmax_ref, forecast_rows),),
        ),
...
def _evidence_payload(
    ...
    base_ts = _datetime_from_packet(evidence_packet, "base_ts")
    forecast_origin_ts = _datetime_from_packet(evidence_packet, "forecast_origin_ts")
    return {
        "packet_id": _stable_id("evidence", run_id, base_ts.isoformat(), forecast_origin_ts.isoformat()),
        "run_id": run_id,
        "base_ts": base_ts,
...
def _datetime_from_packet(packet: Mapping[str, Any], key: str) -> datetime:
    value = packet[key]
    if isinstance(value, datetime):
        return value
    if isinstance(value, str):
        return datetime.fromisoformat(value.replace("Z", "+00:00"))
    raise TypeError(f"{key} must be datetime or ISO string")
