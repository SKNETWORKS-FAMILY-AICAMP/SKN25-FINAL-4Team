...
def _ensure_monthly_table(conn):
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS ops.monthly_report (
...
def aggregate_monthly(
    ...
    """CMS 데이터를 월별로 집계해 monthly_report 테이블에 저장."""
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).parent.parent.parent))
    ...
    try:
        with _db_conn() as conn:
            ...
            cur = conn.cursor()
            cur.execute("SELECT TO_CHAR(timestamp, 'YYYY-MM'), COUNT(*) FROM ops.legacy_anomaly GROUP BY 1;")
            anomaly_counts = {r[0]: r[1] for r in cur.fetchall()}
            for _, row in monthly.iterrows():
                period = str(row["period"])
                cur.execute("""
                    INSERT INTO ops.monthly_report
...
def _generate_trend_narrative(items: list[dict]) -> str:
    ...
    fs_user = (
        "당신은 CMS Agent — 에너지 월간 트렌드 분석 AI입니다.\n"
    ...
    prompt = f"""당신은 CMS Agent — 에너지 월간 트렌드 분석 AI입니다.
시설: Honda R&D Europe GmbH, 독일 오펜바흐. 전력 용어는 "계통 전력"만 사용.
기준: 자급률 6년평균 39.6%, COP 중앙값 2.06.
...
def get_report(
    ...
    # 냉방-외기온 상관 데이터: CMS DB에서 월별 avg(cool_output_P), avg(Ta) 조회
    cooling_vs_temp = []
    try:
        from cms.service.legacy_data.loader import load_range
        if items:
...
def download_monthly_report(
    ...
    from cms.service import report_export
...
def _ensure_daily_table(conn):
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS ops.daily_report (
...
def latest_data_date() -> str | None:
    ...
    try:
        from cms.service.legacy_data.loader import get_data_range
...
def _daily_anomaly_count(conn, date_str: str) -> int:
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT COUNT(*) FROM ops.legacy_anomaly WHERE DATE(timestamp) = %s;",
            (date_str,),
        )
...
def _generate_daily_summary(kpi: dict) -> tuple[str, str]:
    ...
    prompt = f"""당신은 CMS Agent — 공장 에너지 일일 운영 브리핑을 작성하는 AI입니다.
...
(오늘 운영자가 실행할 체크리스트. 이상탐지 건이 있으면 해당 설비 점검을 포함.
 ...
 최대 3개 항목, 각 항목은 "· "(중간점 1개+공백) 으로만 시작. "··"처럼 두 번 쓰지 마세요.)"""
    ...
    fs1_user = (
        "당신은 CMS Agent — 공장 에너지 일일 운영 브리핑을 작성하는 AI입니다.\n"
        "시설: Honda R&D Europe GmbH, 독일 오펜바흐. 전력 용어는 '계통 전력'만 사용.\n\n"
        "## 2024-01-10 일일 KPI\n"
        "- 총 소비: 38,500 kWh\n- 자급률: 42.1%  (기준: 39.6%)\n"
        "- 평균 COP: 2.31  (기준: 2.06)\n- 그리드 의존도: 57.9%\n"
...
def _fetch_daily(conn, date_str: str) -> dict | None:
    ...
    cur.execute("""
        ...
        FROM ops.daily_report WHERE date = %s;
    """, (date_str,))
    r = cur.fetchone()
    if not r:
        return None
...
_OPS_REPORT_CADENCES = {"daily", "weekly", "monthly"}


def _invalid_ops_cadence(cadence: str) -> dict:
    return {
        "cadence": cadence,
        "item": None,
        "error": "지원하지 않는 보고서 주기입니다.",
        "allowed": sorted(_OPS_REPORT_CADENCES),
    }


def _normalize_ops_document_item(item: dict | None) -> dict | None:
    if not item:
        return None
    report_json = item.get("report_json") if isinstance(item.get("report_json"), dict) else {}
    return {
        **item,
        "period": item.get("period") or item.get("period_key"),
        "summary": item.get("executive_summary") or item.get("summary"),
        "operator_actions": report_json.get("operator_actions", item.get("operator_actions", [])),
...
def _fetch_ops_report_row(conn, cadence: str, period_key: str | None = None) -> dict | None:
    from cms.workflow.reports.generation import fetch_latest_report, fetch_report_period

    if period_key:
        return fetch_report_period(conn, cadence, period_key)  # type: ignore[arg-type]
    return fetch_latest_report(conn, cadence)  # type: ignore[arg-type]


def _ops_report_period_column(cadence: str) -> str:
    return "date" if cadence == "daily" else "period"


def _ops_report_table(cadence: str) -> str:
    return {
...
@router.get("/ops/{cadence}/latest")
def get_ops_report_latest(cadence: str):
    """서버 report ops latest route와 로컬 source route를 맞추는 호환 조회."""
    if cadence not in _OPS_REPORT_CADENCES:
        return _invalid_ops_cadence(cadence)

    try:
        with _db_conn() as conn:
            item = _fetch_ops_report_row(conn, cadence)
        if item:
            return {"cadence": cadence, "item": _normalize_ops_document_item(item), "source": _ops_report_table(cadence)}
    except Exception:
        pass
    ...
    if cadence == "daily":
        ...
        if not target:
            return {"cadence": cadence, "item": None, "error": "데이터 날짜 확인 불가"}
        try:
            with _db_conn() as conn:
                _ensure_daily_table(conn)
                item = _fetch_daily(conn, target)
        except Exception as e:
            return {"cadence": cadence, "item": None, "error": safe_err(e)}
        return {"cadence": cadence, "item": _enrich_events(item) if item else None}

    if cadence == "monthly":
        payload = get_report(months=1, skip_ai=True)
        items = payload.get("items") or []
        item = items[-1] if items else None
        return {"cadence": cadence, "item": item, "source": "monthly_report"}

    return {
        "cadence": cadence,
        "item": None,
        "error": "주간 보고서 저장 기준이 아직 로컬 소스에 연결되지 않았습니다.",
    }


@router.get("/ops/{cadence}/periods")
def list_ops_report_periods(cadence: str, limit: int = Query(12, ge=1, le=100)):
    """사용자용 report document 기간 목록."""
    if cadence not in _OPS_REPORT_CADENCES:
        return _invalid_ops_cadence(cadence)
    try:
        table = _ops_report_table(cadence)
        period_column = _ops_report_period_column(cadence)
        with _db_conn() as conn:
            cur = conn.cursor()
            cur.execute(
                f"""
        ...
        return {"cadence": cadence, "items": items, "source": table}
    except Exception as e:
        return {"cadence": cadence, "items": [], "error": safe_err(e)}


@router.get("/ops/{cadence}/periods/{period_key}")
def get_ops_report_period(cadence: str, period_key: str):
    """사용자용 report document 단일 기간 조회."""
    if cadence not in _OPS_REPORT_CADENCES:
        return _invalid_ops_cadence(cadence)
    try:
        with _db_conn() as conn:
            item = _fetch_ops_report_row(conn, cadence, period_key)
        return {"cadence": cadence, "period_key": period_key, "item": _normalize_ops_document_item(item)}
    except Exception as e:
        return {"cadence": cadence, "period_key": period_key, "item": None, "error": safe_err(e)}


@router.get("/ops/{cadence}/periods/{period_key}/markdown")
def get_ops_report_markdown(cadence: str, period_key: str):
    """사용자용 report document Markdown 본문 조회."""
    payload = get_ops_report_period(cadence, period_key)
    item = payload.get("item") if isinstance(payload, dict) else None
    if not item:
        return Response(content="", media_type="text/markdown", status_code=404)
    return Response(content=item.get("markdown") or "", media_type="text/markdown")


@router.post("/ops/{cadence}/generate")
def generate_ops_report(
    cadence: str,
    reference_ts: str | None = Query(None, description="기준 시각 또는 날짜"),
):
    """서버 report ops generate route와 로컬 source route를 맞추는 호환 생성."""
    if cadence not in _OPS_REPORT_CADENCES:
        return _invalid_ops_cadence(cadence)
    ...
    try:
        ...
        from cms.workflow.reports.generation import generate_and_store_report

        now = None
        if reference_ts:
            parsed = datetime.fromisoformat(reference_ts.replace("Z", "+00:00"))
            now = parsed if parsed.tzinfo else parsed.replace(tzinfo=ZoneInfo("Asia/Seoul"))
        elif cadence == "daily":
            target = latest_data_date()
            if target:
                now = datetime.fromisoformat(target).replace(tzinfo=ZoneInfo("Asia/Seoul")) + timedelta(days=1)
        with _db_conn() as conn:
            result = generate_and_store_report(conn, cadence=cadence, readiness={"ok": True, "blocked": False}, now=now)  # type: ignore[arg-type]
            item = _fetch_ops_report_row(conn, cadence, str(result["report"]["period"]))
        return {"cadence": cadence, "item": _normalize_ops_document_item(item), "stored": result["report"], "source": _ops_report_table(cadence)}
    except Exception as e:
        return {"cadence": cadence, "item": None, "error": safe_err(e)}
...
def get_latest_data_date():
    """CMS 데이터에 존재하는 가장 최근 완전한 날짜 반환 (UI 기본값·스케줄러 기준)."""
    ...
    try:
        with _db_conn() as conn:
            ...
            cur.execute("""
                ...
                FROM ops.daily_report ORDER BY date DESC LIMIT %s;
            """, (limit,))
            rows = cur.fetchall()
    except Exception as e:
...
def get_scheduler_status():
    ...
    from cms.service.scheduler import scheduler_status
...
def _generate_balance_narrative(items_with_balance: list[dict]) -> str:
    ...
    prompt = f"""당신은 CMS Agent — 에너지 데이터 품질 분석 AI입니다.
...
def _generate_ei_narrative(items: list[dict], ei_avg: float | None) -> str:
    ...
    prompt = f"""당신은 CMS Agent — 외기온 정규화 에너지 원단위(EI) 분석 AI입니다.
시설: Honda R&D Europe GmbH, 독일 오펜바흐.
    ...
    try:
        with _db_conn() as conn:
            ...
            cur.execute("""
                ...
                FROM ops.monthly_report ORDER BY period DESC LIMIT %s;
            """, (months,))
            rows = cur.fetchall()
    except Exception as e:
        return {"error": safe_err(e), "items": [], "narrative": ""}
...
def _generate_billing_narrative(billing: dict) -> str:
    ...
    prompt = f"""당신은 CMS Agent — 공장 에너지 비용 분석 AI입니다.
시설: Honda R&D Europe GmbH, 독일 오펜바흐.
