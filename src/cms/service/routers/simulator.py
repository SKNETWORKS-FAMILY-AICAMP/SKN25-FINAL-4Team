...
async def _do_check():
    ...
    if last_checked.date() != window_end.date():
        ...
        try:
            from cms.service.routers.control import evaluate_pending_outcomes
    ...
    try:
        ...
        if is_available():
            ...
            if not res.empty:
                ...
                try:
                    from cms.service.routers.settings import get_alert_min_level
                ...
                # ops.legacy_anomaly 테이블에도 저장 (새 구간만)
...
def _ensure_daily_report(date_str: str) -> None:
    ...
    """
    try:
        from cms.service.routers.report import _fetch_daily, build_daily_report
...
def _persist_residual_results(df) -> None:
    """잔차 모델 결과를 ops.legacy_anomaly 테이블에 저장."""
    try:
        from cms.service.db import get_conn
        import psycopg2
