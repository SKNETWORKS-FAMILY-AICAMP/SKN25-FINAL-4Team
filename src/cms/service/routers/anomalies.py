...
def list_anomalies(
    ...
    try:
        with _db_conn() as conn:
            ...
            cur.execute(f"SELECT COUNT(*) FROM ops.legacy_anomaly {where}", params)
...
def anomaly_context(anomaly_id: int, hours: int = Query(24, ge=6, le=72)):
    ...
    try:
        with _db_conn() as conn:
            ...
            cur.execute("""
                SELECT id, timestamp, meter_id, anomaly_type, severity, description,
                       score_stat, score_iso, score_lstm, vote_count
                FROM ops.legacy_anomaly WHERE id = %s;
            """, (anomaly_id,))
            row = cur.fetchone()
    except Exception as e:
    ...
    # 2. 전후 시계열 로드
    try:
        from cms.service.legacy_data.loader import load_reduced
...
def _migrate_schema(conn) -> None:
    """ops.legacy_anomaly 테이블에 신 모델용 컬럼 추가 (없으면)."""
    ...
    for col, dtype in [("actual_w", "FLOAT"), ("predicted_w", "FLOAT"),
        ...
        cur.execute(f"""
            ALTER TABLE ops.legacy_anomaly
            ADD COLUMN IF NOT EXISTS {col} {dtype};
        """)
    cur.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_anomaly_unique
        ON ops.legacy_anomaly (timestamp, meter_id);
    """)
    conn.commit()
...
def _save_residual_results(anomalies, job_id: str) -> int:
    """LSTM 잔차 탐지 결과를 ops.legacy_anomaly에 저장."""
    import pandas as pd
    try:
        with _db_conn() as conn:
            ...
            for _, row in anomalies.iterrows():
                ...
                cur.execute("""
                    INSERT INTO ops.legacy_anomaly
...
def _do_detection(job_id: str, start: str, end: str, loop=None) -> None:
    ...
    try:
        ...
        if not is_available():
            ...
            from cms.modeling.legacy_models.anomaly.ensemble import run
        ...
        if loop and ("CRITICAL" in counts or "HIGH" in counts):
            import asyncio
            from cms.service.routers.notifications import manager as notification_manager
