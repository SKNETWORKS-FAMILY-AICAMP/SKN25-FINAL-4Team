...
def _anchor_now(conn) -> datetime:
    ...
    cur = conn.cursor()
    cur.execute("SELECT MAX(timestamp) FROM ops.legacy_anomaly;")
...
def _latest_metrics(anchor: datetime) -> dict:
    ...
    try:
        ...
        if df is None or df.empty:
            return out
        from cms.service.config import get_equipment_list
...
def _elec_meters(eq_id: str) -> list[str]:
    from cms.service.legacy_data.loader import (
...
def run_diagnosis(eq_id: str, window_days: int = _WINDOW_DAYS, regenerate: bool = False) -> dict:
    ...
    try:
        ...
        from cms.knowledge.domain_knowledge import ANOMALY_DOMAIN_PROMPT
        system = (
...
def _ensure_anomaly_columns(conn) -> None:
    """ops.legacy_anomaly에 v84 컬럼이 없으면 추가 (최초 1회)."""
    cur = conn.cursor()
    for col, dtype in [("actual_w", "FLOAT"), ("predicted_w", "FLOAT"),
                       ("residual_w", "FLOAT"), ("source", "TEXT"), ("gateway_failure", "BOOLEAN")]:
        cur.execute(f"ALTER TABLE ops.legacy_anomaly ADD COLUMN IF NOT EXISTS {col} {dtype};")
    conn.commit()
...
def compute_predictive(months: int = 8) -> dict:
    ...
    with _db_conn() as conn:
        ...
        # 냉방: 월평균 COP 추세
        cur.execute(
            "SELECT period, avg_cop FROM ops.monthly_report "
        ...
        # 전 설비: 월별 이상 건수 추세
        from cms.service.config import get_equipment_list as _get_equipment_list
...
def insert_work_order(equipment_id=None, equipment_name=None, title=None,
    ...
    with _db_conn() as conn:
        ...
        cur.execute(
            f"""
            INSERT INTO ops.work_order (equipment_id, equipment_name, title, cause, action, priority)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING {_WO_COLS};
            """,
...
def list_work_orders(
    ...
    with _db_conn() as conn:
        ...
        cur.execute(
            f"""
            SELECT {_WO_COLS} FROM ops.work_order
            {where}
