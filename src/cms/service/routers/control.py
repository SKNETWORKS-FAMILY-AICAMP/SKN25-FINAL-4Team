...
def _ensure_control_table(conn):
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS ops.control_action (
            id              TEXT PRIMARY KEY,
    ...
    for col, ddl in [
        ...
        cur.execute(f"ALTER TABLE ops.control_action ADD COLUMN IF NOT EXISTS {col} {ddl};")
    conn.commit()
    _table_ready = True
    ...
    try:
        with _db_conn() as conn:
            ...
            if recs:
                ...
                cur = conn.cursor()
                cur.execute(
                    "SELECT id, status, status_updated FROM ops.control_action WHERE id = ANY(%s);",
                    (ids,)
                )
...
def clear_recommendations():
    ...
    try:
        with _db_conn() as conn:
            cur = conn.cursor()
            _ensure_control_table(conn)
            cur.execute("DELETE FROM ops.control_action;")
...
def _save_status(rec_id: str, status: str, body: dict | None) -> None:
    ...
    try:
        with _db_conn() as conn:
            ...
            cur = conn.cursor()
            cur.execute("""
                INSERT INTO ops.control_action
                    (id, category, priority, title, description, status, status_updated, sim_now)
                VALUES (%s, %s, %s, %s, %s, %s, NOW(), %s)
                ON CONFLICT (id) DO UPDATE SET
                    status         = EXCLUDED.status,
                    status_updated = NOW(),
                    sim_now        = COALESCE(ops.control_action.sim_now, EXCLUDED.sim_now);
...
def _compute_outcome(conn, category: str, rec_sim_now, status: str) -> dict:
    ...
    try:
        ...
        if category == "peak_shift":
            ...
            cur = conn.cursor()
            cur.execute("""
                SELECT COUNT(*) FROM ops.legacy_anomaly
