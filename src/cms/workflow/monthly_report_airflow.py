"""Monthly readiness-first CMS report generation Airflow DAG contract."""

from __future__ import annotations

from cms.workflow import report_readiness_airflow as base
from cms.workflow.airflow_runtime_policy import AIRFLOW_MONTHLY_REPORT_SCHEDULE
...
AIRFLOW_INTEGRATION_LANES = ("monthly_report", "qa_evidence", "promotion_check", "job_metadata")
TASK_IDS = base.TASK_IDS
PROBE_ENV_FLAG = base.PROBE_ENV_FLAG

def describe_dag() -> base.ReportReadinessAirflowDag:
    return base.describe_dag(dag_id=DAG_ID, schedule=AIRFLOW_MONTHLY_REPORT_SCHEDULE, report_kind=REPORT_KIND)
...
def airflow_task_entrypoint(task_id: str, **context: object) -> dict[str, object]:
    return base.airflow_task_entrypoint(task_id, report_kind=REPORT_KIND, dag_id=DAG_ID, schedule=AIRFLOW_MONTHLY_REPORT_SCHEDULE, **context)

def make_airflow_dag(*, enabled: bool = AIRFLOW_DAG_ENABLED) -> object:
    return base.make_airflow_dag(enabled=enabled, dag_id=DAG_ID, schedule=AIRFLOW_MONTHLY_REPORT_SCHEDULE, report_kind=REPORT_KIND)
