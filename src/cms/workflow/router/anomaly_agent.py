...
from cms.knowledge.domain_knowledge import ANOMALY_DOMAIN_PROMPT, ANOMALY_RECOMMENDATION_PROMPT
...
def _connect_db():
    """Lazily import psycopg2 so router imports work without DB extras."""
    ...
    return psycopg2.connect(
        ...
        dbname=os.getenv("DB_NAME", "ems"),
        user=os.getenv("DB_USER", "postgres"),
        password=os.getenv("DB_PASS") or os.getenv("DB_PASSWORD", ""),
    )


def _safe_table_name(env_name: str, default: str) -> str:
...
ANOMALY_WARNING_TABLE = _safe_table_name("CMS_ROUTER_ANOMALY_WARNING_TABLE", "mart.anomaly_warning_1h")
LEGACY_ANOMALY_RESULTS_TABLE = _safe_table_name("CMS_ROUTER_LEGACY_ANOMALY_RESULTS_TABLE", "anomaly_results")
...
def _run_ml_anomaly(start: str, end: str) -> list[dict]:
    ...
    try:
        from cms.modeling.legacy_models.anomaly.residual_model import predict_anomaly, is_available
        if not is_available():
            return []
...
def _merge_count_rows(rows) -> dict:
    result: dict = {}
    for atype, sev, cnt in rows:
...
def _count_anomaly_warnings_by_type(start: str | None, end: str | None) -> dict | None:
    """Current CMS contract: mart.anomaly_warning_1h read path."""

    try:
        conn = _connect_db()
        try:
            cur = conn.cursor()
            conds, params = ["warning_flag = TRUE"], []
            if start:
                conds.append("target_ts >= %s"); params.append(start)
            if end:
                conds.append("target_ts <  %s"); params.append(end)
            where = "WHERE " + " AND ".join(conds)
            cur.execute(f"""
                SELECT
                    COALESCE(NULLIF(warning_reason_code, 'NONE'), warning_type) AS anomaly_type,
                    CASE
                        WHEN warning_type = 'high' THEN 'HIGH'
                        WHEN warning_type = 'low' THEN 'MEDIUM'
                        ELSE 'LOW'
                    END AS severity,
                    COUNT(*)
                ...
                ORDER BY COUNT(*) DESC;
            """, params)
            return _merge_count_rows(cur.fetchall())
        finally:
            conn.close()
    except Exception:
        return None
...
def _count_legacy_anomalies_by_type(start: str | None, end: str | None) -> dict:
    try:
        conn = _connect_db()
        try:
            cur = conn.cursor()
            ...
            cur.execute(f"""
                ...
                FROM {LEGACY_ANOMALY_RESULTS_TABLE} {where}
...
def _count_anomalies_by_type(start: str | None, end: str | None) -> dict:
    """기간 내 anomaly_type별 전체 건수 집계 (current mart table, legacy fallback)."""

    current = _count_anomaly_warnings_by_type(start, end)
    if current is not None:
        return current
    return _count_legacy_anomalies_by_type(start, end)


def _fetch_anomaly_warnings(limit: int, start: str | None, end: str | None) -> list[dict] | None:
    """Current CMS contract: fetch warnings from mart.anomaly_warning_1h."""
    ...
    try:
        ...
        try:
            ...
            where = "WHERE " + " AND ".join(conds)
            cur.execute(f"""
                SELECT target_ts, meter_urn, warning_type, warning_reason_code,
                       lead_step, predicted_p, threshold_lower, threshold_upper
                FROM {ANOMALY_WARNING_TABLE}
                {where}
            ...
            return [
                {
                    "timestamp": r[0],
                    "meter_id": r[1],
                    "type": r[3] if r[3] and r[3] != "NONE" else r[2],
                    "severity": "HIGH" if r[2] == "high" else "MEDIUM",
                    "description": f"{r[2]} anomaly warning ({r[3]})",
                    "votes": r[4],
                    "actual_w": None,
                    "predicted_w": r[5],
                    "residual_w": None,
...
def _fetch_legacy_anomalies(limit: int, start: str | None, end: str | None, exclude_gateway: bool) -> list[dict]:
    try:
        conn = _connect_db()
        try:
            cur  = conn.cursor()
            ...
            cur.execute(f"""
                ...
                FROM {LEGACY_ANOMALY_RESULTS_TABLE}
                {where}
...
def _fetch_anomalies(limit: int = 20,
    ...
    """이상 경고 조회. Current mart.anomaly_warning_1h 우선, legacy anomaly_results fallback."""

    current = _fetch_anomaly_warnings(limit, start, end)
    if current is not None:
        return current
    return _fetch_legacy_anomalies(limit, start, end, exclude_gateway)
...
def run(state: dict) -> dict:
    ...
    source_str = "LSTM 잔차 (실시간)" if ml_source else f"{ANOMALY_WARNING_TABLE} DB (legacy fallback 가능)"
    ...
    prompt = f"""당신은 CMS Agent — 에너지 시설 이상을 감지하고 운영자에게 조치를 안내하는 AI입니다.
