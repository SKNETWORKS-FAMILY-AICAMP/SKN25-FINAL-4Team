...
from cms.workflow.router.anomaly_agent import _REGIME_EVENTS, _GATEWAY_FAILURES
...
def _connect_db():
    """Lazily import psycopg2 so reporting imports work without DB extras."""
...
**중요: 위 KPI 데이터에 없는 수치를 만들어내지 마세요. 수치가 없으면 "데이터 없음"으로 표기하세요.**

**하나의 완성된 보고서만 작성하세요. 대안, 다른 버전, 예시 비교를 절대 제시하지 마세요.**
아래 형식으로 보고서를 작성하세요. **각 섹션은 3줄 이내**로 간결하게 작성합니다.

## 1. 핵심 요약
(2~3줄 이내 — 가장 중요한 변화 1~2가지만)

## 2. KPI 분석
(수치와 단위 명시, 전기 용어는 "계통 전력" 사용 — 3줄 이내)

## 3. 이상탐지 현황
(anomaly_count 기반 — 데이터 없으면 "별도 조회 필요" 명시 — 2줄 이내)

## 4. 개선 권고사항
(구체적 수치 포함, 확실하지 않으면 추측 명시 — 3줄 이내)

예시 출력 형식:
## 1. 핵심 요약
이번 달 소비량이 전월 대비 5.2% 감소하며 효율이 개선됐습니다. 자급률은 32.1%로 목표(30%)를 초과 달성했습니다.
## 2. KPI 분석
총 소비량 1,240 MWh(전월 1,308 MWh, -5.2%). 자급률 32.1%, COP 3.42로 전월 대비 개선됐습니다.
## 3. 이상탐지 현황
이번 달 이상 3건(HIGH 1건, MEDIUM 2건). HIGH 1건은 2024-03-15 전류 스파이크로 즉시 조치 완료됐습니다.
## 4. 개선 권고사항
역률이 0.82로 목표(0.85)에 미달합니다. 역률개선 콘덴서 점검을 권고합니다."""
