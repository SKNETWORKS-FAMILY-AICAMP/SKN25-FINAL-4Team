...
def _llm_chat(messages: list[dict], *, max_tokens: int = 1024, fast: bool = False) -> str:
    try:
        from cms.workflow.router.llm_client import chat
    except ModuleNotFoundError:
        return "rag"

    return chat(messages, max_tokens=max_tokens, fast=fast)
...
def rag_node(state: AgentState) -> AgentState:
    from cms.workflow.router import rag_agent
...
def anomaly_node(state: AgentState) -> AgentState:
    from cms.workflow.router import anomaly_agent
...
def report_node(state: AgentState) -> AgentState:
    from cms.workflow.router import reporting_agent
...
def forecast_node(state: AgentState) -> AgentState:
    from cms.workflow.router import forecast_agent
...
def cms_node(state: AgentState) -> AgentState:
    from cms.workflow.router import cms_agent
