...
def _side_effects_enabled(ctx: dict) -> bool:
    """Runtime boundary: imported router actions stay disabled unless approved and explicitly enabled."""

    return bool(ctx.get("approved")) and os.getenv("CMS_ROUTER_ENABLE_SIDE_EFFECTS", "").lower() in {"1", "true", "yes", "on"}


def _approval_required_message(action: str) -> str:
    return (
        f"{action} 요청은 승인 게이트가 필요합니다. "
        "기본 라우터/리뷰 경로에서는 DB 또는 시뮬레이터 상태를 변경하지 않았습니다. "
        "승인된 ops/workflow 작업으로 실행해 주세요."
    )
...
def _detect_equipment(q: str) -> str | None:
    ql = q.lower()
    from cms.service.config import get_eq_keywords
...
def _fmt_work_orders() -> str:
    ...
    from cms.service.routers.cms import _wo_row, _WO_COLS
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT status, COUNT(*) FROM ops.work_order GROUP BY status;")
        by = {"open": 0, "in_progress": 0, "done": 0}
        for s, c in cur.fetchall():
            by[s] = c
        cur.execute(
            f"""SELECT {_WO_COLS} FROM ops.work_order
...
def _fmt_status_overall() -> str:
    from cms.service.routers.cms import compute_equipment_status
...
def _fmt_predictive() -> str:
    from cms.service.routers.cms import compute_predictive
...
def _fmt_diagnosis(eq_id: str) -> str:
    from cms.service.routers.cms import run_diagnosis, compute_equipment_status
...
def _do_create_work_order(eq_id: str | None) -> str:
    ...
    from cms.service.routers.cms import (
...
def run(state: dict) -> dict:
    ...
    # ── 행동(액션) 우선 ──
    if _ACT_WO.search(q):
        answer = _do_create_work_order(eq_id) if _side_effects_enabled(ctx) else _approval_required_message("작업지시 생성")
    elif _ACT_SIM.search(q):
        answer = _do_simulator(q) if _side_effects_enabled(ctx) else _approval_required_message("시뮬레이터 제어")
    # ── 조회 ──
