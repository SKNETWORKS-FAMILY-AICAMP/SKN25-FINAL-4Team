...
CMS's first scheduler target set: scheduled, read-only daily/weekly/monthly
report DAGs plus manual paused no-write registrations for model-serving,
...
AIRFLOW_REPORT_READINESS_REGISTRATION_MODE = "scheduled_read_only_report"
AIRFLOW_REPORT_READINESS_SCHEDULE = AIRFLOW_DAILY_REPORT_SCHEDULE
AIRFLOW_REPORT_READINESS_TIMEZONE = "Asia/Seoul"
AIRFLOW_REPORT_READINESS_TRIGGER_POLICY = "scheduled_report_read_only"
...
AIRFLOW_CONNECTION_IDS = MappingProxyType(
    {
        ...
        "mongo": "cms_mongo",
        "kafka": "cms_kafka",
        "model_artifacts": "cms_model_artifacts",
...
def describe_all_runtime_decisions() -> tuple[AirflowRuntimeDecision, ...]:
    """Describe all CMS Airflow DAG deployment policies."""
    ...
    return (
        ...
        describe_runtime_decision(airflow_skeleton),
        describe_runtime_decision(champion_airflow_skeleton),
        describe_runtime_decision(model_serving_airflow_skeleton),
    )
...
def _infer_connection_ids(resources: tuple[str, ...], *, task_id: str) -> tuple[str, ...]:
    ...
    for resource in resources:
        ...
        if "artifact" in resource or resource.startswith("model_registry."):
            connection_ids.append(AIRFLOW_CONNECTION_IDS["model_artifacts"])
