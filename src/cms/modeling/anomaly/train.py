...
실행:
  conda run -n skn25 python -m cms.modeling.anomaly.train --horizon 1
  conda run -n skn25 python -m cms.modeling.anomaly.train --horizon 3
"""
...
PROJECT_ROOT = Path(os.getenv("CMS_PROJECT_ROOT", str(Path(__file__).resolve().parents[4]))).resolve()
...
from cms.modeling.anomaly.config import (
    ...
    USE_RESIDUAL_TARGET,
    MeterSpec,
    TRAINING_METER_SPECS,
    training_specs_for_group,
)
from cms.modeling.anomaly.db import build_engine, fetch_meter_frame
from cms.modeling.anomaly.local_data import fetch_meter_frame_from_dir
from cms.modeling.anomaly.preprocessing import build_windows, prepare_model_frame
from cms.modeling.anomaly.model import (
    RecurrentPredictor,
    add_anomaly_flags,
...
from cms.modeling.anomaly.catboost_model import fit_catboost, predict_catboost_scaled
from cms.modeling.anomaly.lightgbm_model import fit_lightgbm, predict_lightgbm_scaled, save_lightgbm
from cms.modeling.anomaly.ridge import fit_ridge, predict_ridge_scaled
from cms.modeling.anomaly.naive import predict_seasonal_naive
from cms.modeling.anomaly.router import v63_route_group
from cms.modeling.anomaly.ensemble import (
    build_prediction_df,
...
from cms.modeling.anomaly.plots import save_plots
...
from cms.modeling.anomaly.preprocessing import make_loader
from cms.modeling.anomaly.config import SMOOTH_L1_BETA
...
def parse_args():
    ...
    p.add_argument("--save-diagnostics", action="store_true",
                   help="분석용 val/test prediction CSV, metrics, intermediate metrics, PNG 저장")
    return p.parse_args()


def _get_specs(args):
    specs = training_specs_for_group()
    training_urns = {s.meter_urn for s in TRAINING_METER_SPECS}
    all_urns = {s.meter_urn for s in ALL_METER_SPECS}
    transfer_urns = {s.meter_urn for s in ELECTRIC_TRANSFER_MEMBER_SPECS}
    if args.groups:
        gs = set(args.groups)
        specs = [s for s in specs if s.group in gs]
    if args.meters:
        requested = set(args.meters)
        unknown = sorted(requested - all_urns)
        transfer_requested = sorted(requested & transfer_urns)
        if unknown:
            raise ValueError("알 수 없는 meter_urn: " + ", ".join(unknown))
        if transfer_requested:
            raise ValueError(
                "학습 대상이 아닌 transfer_member가 포함됨: "
                + ", ".join(transfer_requested)
                + ". 해당 계량기는 대표 model_urn artifact를 사용하므로 직접 학습하지 않습니다."
            )
        invalid = sorted(requested - training_urns)
        if invalid:
            raise ValueError("학습 대상이 아닌 meter_urn: " + ", ".join(invalid))
        specs = [s for s in specs if s.meter_urn in requested]
    return specs
...
def _make_bundle(raw, spec, horizon, window_size, use_time_features):
    from cms.modeling.anomaly.preprocessing import prepare_model_frame, build_windows
