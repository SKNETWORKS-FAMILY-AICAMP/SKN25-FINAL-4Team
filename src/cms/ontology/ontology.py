...
class EMSOntology:
    ...
    def from_default(cls, root: str | Path | None = None) -> EMSOntology:
        ...
        return cls.from_ttl(project_root / "docs/ontology/ems.ttl", root=project_root)
    ...
    def get_meter_context(self, meter_urn: str) -> dict[str, object]:
        ...
        building = self._single_uri(meter, EMS.locatedInBuilding)
        role = self._single_uri(meter, EMS.hasRole)
        return {
            "meter_urn": self._literal(meter, EMS.meterUrn),
            "domain": self._literal(meter, EMS.meterDomain),
            "role": self._resource_code(role, EMS.meterRoleCode),
            "group": self._resource_code(group, EMS.equipmentGroupCode),
            "building": self._resource_code(building, EMS.buildingCode),
            "equipment_name": self._literal(meter, EMS.equipmentName, default=""),
            "sign_convention": self._literal(meter, EMS.signConvention),
            "anomaly_priority": int(self._literal(meter, EMS.anomalyPriority)),
    ...
    def get_feature_meter_set(
        ...
        *,
        ...
        if domain is not None:
            meters = [meter for meter in meters if domain in self._literal_values(meter, EMS.meterDomain)]
        if role is not None:
    ...
    def get_aggregate_policy(self, *, group: str, domain: str, role: str) -> dict[str, object]:
        ...
        return {
            ...
            "redundancy_policies": self._local_names(self._subjects(RDF.type, EMS.RedundancyPolicy)),
            "feature_rules": self._local_names(self._subjects(RDF.type, EMS.FeatureRule)),
        }
    ...
    def _meter_urns(self, meters: Iterable[URIRef]) -> list[str]:
        urns = [self._literal(meter, EMS.meterUrn) for meter in meters if isinstance(meter, URIRef) and (meter, RDF.type, EMS.Meter) in self.graph]
    ...
    def _local_names(resources: Iterable[URIRef]) -> list[str]:
        return sorted(str(resource).rsplit("/", 1)[-1] for resource in resources)
