...
import { Bot, AlertTriangle, Copy, Check, MapPin, Download, Folder, X, Zap } from 'lucide-react'
...
export default function ChatPanel({ initialSessionId = null, initialMessages = null, readOnly = false, onSessionCreated = null, context = null, onClearContext = null, seedQuestion = null, onSeedConsumed = null, onNavigate = null } = {}) {
  const [messages,  setMessages]  = useState(initialMessages || [])
  const [input,     setInput]     = useState('')
  const [loading,   setLoading]   = useState(false)
  const [sessionId, setSessionId] = useState(() => initialSessionId ?? (!readOnly && localStorage.getItem('chat_session_id')) ?? null)
  ...
  const submit = useCallback(async (question) => {
    ...
    try {
      ...
      setMessages(prev => {
        const next = prev.filter(m => !m.streaming)
        return [...next, { role: 'error', text: '서버 오류가 발생했습니다. AI 서비스가 일시적으로 응답하지 않고 있습니다.' }]
      })
    } finally {
      setLoading(false)
    }
    ...
    <div style={s.wrap}>
      ...
      <div style={s.chatHeader}>
        <span style={s.chatTitle}>
          {readOnly ? <><Folder size={13} style={{verticalAlign:'-2px',marginRight:4}}/>과거 대화 보기</> : <><Bot size={13} style={{verticalAlign:'-2px',marginRight:4}}/>에너지 분석 채팅</>}
        </span>
      ...
      {!readOnly && context?.equipmentName && (
        <div style={s.ctxBar}>
          <MapPin size={11} style={{verticalAlign:'-1px',marginRight:4}}/>현재 설비: <b style={{ color: 'var(--text)' }}>{context.equipmentName}</b>
          <span style={{ color: 'var(--text4)' }}> — "진단해줘 / 작업지시 만들어줘"에 자동 적용</span>
          {onClearContext && (
            <button onClick={onClearContext} style={s.ctxClear} title="컨텍스트 해제"><X size={10}/></button>
          )}
        </div>
      )}
      ...
      <div style={s.messages}>
        {messages.length === 0 && (
          <div style={s.empty}>
            <div style={s.emptyIcon}><Zap size={32} strokeWidth={1.5}/></div>
        ...
        {messages.map((m, i) => (
          <div key={i} style={{ ...s.row, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            {m.role !== 'user' && (
              <div style={s.avatar}>{m.role === 'error' ? <AlertTriangle size={14}/> : <Bot size={14}/>}</div>
            )}
            <div style={{
              ...s.bubble,
                      ...
                      : <>
                          ...
                          {m.pdfFilename && (
                            <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 10, display: 'flex', justifyContent: 'flex-start' }}>
                              ...
                              >
                                <Download size={11} style={{verticalAlign:'-1px',marginRight:4}}/>PDF 리포트 다운로드
                              </a>
                            </div>
                          )}
                        </>
        ...
        {loading && !messages.some(m => m.streaming) && (
          <div style={{ ...s.row, justifyContent: 'flex-start' }}>
            <div style={s.avatar}><Bot size={14}/></div>
      ...
      {/* 빠른 페이지 이동 */}
      {!readOnly && onNavigate && (
        <div style={{ padding: '6px 16px 0', display: 'flex', gap: 6, flexWrap: 'wrap', flexShrink: 0 }}>
          <span style={{ fontSize: 10, color: 'var(--text4)', alignSelf: 'center', marginRight: 2 }}>바로가기</span>
          {[
            { id: 'dashboard',   label: '대시보드' },
            { id: 'equipment',   label: '설비 감시' },
            { id: 'anomaly',     label: '이상탐지' },
            { id: 'forecast',    label: '수요예측' },
            { id: 'control',     label: '운영 권고' },
            { id: 'report',      label: '에너지 분석' },
          ].map(({ id, label }) => (
            <button key={id} onClick={() => onNavigate(id)}
              style={{ padding: '3px 10px', background: 'var(--surface)', border: '1px solid var(--border)',
                borderRadius: 12, fontSize: 11, color: 'var(--text3)', cursor: 'pointer', fontWeight: 500 }}>
              {label}
            </button>
          ))}
        </div>
      ...
      {!readOnly && (
        <div style={s.inputWrap}>
          <textarea
            ref={inputRef}
            style={{ ...s.input, resize: 'none', overflow: 'hidden', minHeight: 42, maxHeight: 120 }}
            value={input}
            rows={1}
            onChange={e => {
            ...
            placeholder="에너지 데이터에 대해 질문하세요... (Shift+Enter 줄바꿈)"
            disabled={loading}
          />
          <button
...
const s = {
  ...
  input:      { flex: 1, padding: '10px 16px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontSize: 14, outline: 'none', lineHeight: 1.5, boxSizing: 'border-box' },
