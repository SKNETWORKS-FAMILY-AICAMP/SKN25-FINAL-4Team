...
import { AlertTriangle, AlertCircle, Search, Calendar } from 'lucide-react'
import {
  ResponsiveContainer,
  PieChart,
  Pie,
...
export default function AnomalyChartPanel() {
  ...
  if (error) {
    return (
      <div style={s.center}>
        <span style={{ color: '#f85149', fontSize: 13, display:'flex', alignItems:'center', gap:4 }}><AlertTriangle size={13}/>{error}</span>
      </div>
    )
  }
  ...
  const renderSummary = () => {
    ...
    return (
      <div style={s.chartBox}>
        <div style={s.chartTitle}><AlertCircle size={13} style={{verticalAlign:'-2px',marginRight:5}}/>심각도 등급별 감지 분포</div>
  ...
  const renderTypes = () => {
    ...
    return (
      <div style={s.chartBox}>
        <div style={s.chartTitle}><Search size={13} style={{verticalAlign:'-2px',marginRight:5}}/>이상 유형별 발생 통계</div>
        <div style={{ width: '100%', height: 260 }}>
          <ResponsiveContainer>
  ...
  const renderTimeline = () => {
    ...
    return (
      <div style={s.chartBox}>
        <div style={s.chartTitle}><Calendar size={13} style={{verticalAlign:'-2px',marginRight:5}}/>월간 이상 감지 누적 추이</div>
        <div style={{ width: '100%', height: 260 }}>
          <ResponsiveContainer>
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.tabHeader}>
        ...
        >
          <AlertCircle size={11} style={{verticalAlign:'-1px',marginRight:3}}/>심각도 비율
        </button>
        <button
          style={{ ...s.tabBtn, ...(activeTab === 'types' ? s.tabBtnActive : {}) }}
          onClick={() => setActiveTab('types')}
        >
          <Search size={11} style={{verticalAlign:'-1px',marginRight:3}}/>유형별 현황
        </button>
        <button
          style={{ ...s.tabBtn, ...(activeTab === 'timeline' ? s.tabBtnActive : {}) }}
          onClick={() => setActiveTab('timeline')}
        >
          <Calendar size={11} style={{verticalAlign:'-1px',marginRight:3}}/>월별 트렌드
        </button>
      </div>
