...
import { TYPE_LABEL } from '../../constants/anomaly'
import { X, Bot, RefreshCw, Factory } from 'lucide-react'
import {
...
import { getAnomalies, getAnomalySummary, getAnomalyTimeline, getAnomalyTypes,
         getAnomalyContext, getAnomalyEvents, sendChat } from '../../api/client'
...
const TYPE_METRICS = {
  ...
  LowLoad: [{ key: 'grid_P', name: '계통 전력 (kW)', color: '#d29922', unit: 'kW' }],
  PhysicalAnomaly: [{ key: 'grid_P', name: '계통 전력 (kW)', color: '#a371f7', unit: 'kW' }],
  Unknown: [
...
function DetailPanel({ item, onClose }) {
  ...
  const anomalyTs = ctx?.anomaly_ts?.slice(0, 16)
  ...
  return (
    <div style={s.detail}>
      ...
      <div style={s.detailHeader}>
        ...
        <button style={s.closeBtn} onClick={onClose}><X size={12}/></button>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={s.detailTs}>{item.timestamp?.slice(0,16).replace('T',' ')}</span>
        {item.meter_id && <span style={{ fontSize: 11, color: 'var(--text4)', padding: '1px 6px', border: '1px solid var(--border)', borderRadius: 4 }}>{item.meter_id}</span>}
      </div>
          ...
          <div key={m.key} style={s.miniChart}>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={ctx.timeseries} margin={{ top: 4, right: 8, left: -28, bottom: 0 }}>
                ...
                {anomalyTs && (
                  <ReferenceLine
                    x={ctx.timeseries.find(d => d.ts?.slice(0,16) === anomalyTs)?.ts}
      ...
      <div style={s.aiSection}>
        ...
        {!ai && (
          <button style={s.aiBtn} onClick={askAI} disabled={aiLoad}>
            {aiLoad ? '분석 중...' : <><Bot size={11} style={{verticalAlign:'-1px',marginRight:4}}/>AI 원인 분석 요청</>}
          </button>
        )}
        {ai && <div style={s.aiText}>{ai}</div>}
      </div>
      {events.length > EV_PAGE && (
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', paddingTop: 4 }}>
          <button style={{ padding: '5px 12px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)', fontSize: 12, cursor: 'pointer' }}
            disabled={page === 0} onClick={() => setPage(p => p - 1)}>← 이전</button>
          <span style={{ fontSize: 12, color: 'var(--text3)', alignSelf: 'center' }}>
            {page + 1} / {Math.ceil(events.length / EV_PAGE)}
          </span>
          <button style={{ padding: '5px 12px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)', fontSize: 12, cursor: 'pointer' }}
            disabled={(page + 1) * EV_PAGE >= events.length} onClick={() => setPage(p => p + 1)}>다음 →</button>
        </div>
      )}
    </div>
  )
...
const TYPE_COLORS = {
  COPDrop: '#2563eb', CHPOutage: '#3fb950', PowerSpike: '#f85149', LowLoad: '#d29922', PhysicalAnomaly: '#a371f7',
...
function EventsView({ severity, year, month, excludeGf }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {events.slice(page * EV_PAGE, (page + 1) * EV_PAGE).map((ev, i) => (
        <div key={i} style={{
...
export default function AnomalyPanel({ equipmentFilter } = {}) {
  ...
  const loadList = () => {
    ...
    Promise.all([
      getAnomalySummary(),
      getAnomalyTimeline(),
      getAnomalies(PAGE, severity || undefined, year || undefined, month || undefined, offset, true, typeParam),
    ]).then(([sv, t, a]) => {
  ...
  useEffect(() => { loadList() }, [severity, year, month, offset, eqFilter]) // eslint-disable-line
    ...
    <div style={s.wrap}>
      ...
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <select style={s.select} value={year} onChange={e => { setYear(e.target.value); setMonth(''); setOffset(0); setSelected(null) }}>
            ...
            {[...new Set(timeline.map(t => t.month?.slice(0,4)).filter(Boolean))].sort().map(y => (
          ...
          <select style={s.select} value={severity} onChange={e => { setSeverity(e.target.value); setOffset(0); setSelected(null) }}>
            <option value="MEDIUM+">HIGH + MEDIUM (기본)</option>
          ...
          <button
            ...
            title="이상탐지 데이터 새로고침"
          >
            {loading ? '...' : <><RefreshCw size={11} style={{verticalAlign:'-1px',marginRight:4}}/>새로고침</>}
          </button>
        </div>
      </div>
      ...
      {eqFilter && (
        <div style={s.filterBanner}>
          <span><Factory size={11} style={{verticalAlign:'-1px',marginRight:4}}/><strong>{eqFilter.name}</strong> 관련 이상만 표시 중
            <span style={{ color: 'var(--text3)', marginLeft: 6 }}>
              ({(eqFilter.types ?? []).map(t => TYPE_LABEL[t] ?? t).join(', ')})
            </span>
          </span>
          <button onClick={() => { setEqFilter(null); setOffset(0) }} style={s.filterClear}><X size={10} style={{verticalAlign:'-1px',marginRight:3}}/>필터 해제</button>
        </div>
      )}

      {viewMode === 'events' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 20px 20px' }}>
          <EventsView severity={severity} year={year} month={month} excludeGf={true} />
        </div>
      )}
      ...
      <div style={{ ...s.body, display: viewMode === 'points' ? 'flex' : 'none' }}>
        ...
        <div style={{ ...s.left, width: selected ? '38%' : '100%' }}>
          {/* 요약 카드 */}
          <div style={s.cards}>
            {[...summary].sort((a,b) => ({'HIGH':0,'MEDIUM':1,'LOW':2}[a.severity]??3)-({'HIGH':0,'MEDIUM':1,'LOW':2}[b.severity]??3)).map(sv => (
              ...
              return (
                ...
                >
                  <div style={s.itemTop}>
                    ...
                    {item.meter_id && <span style={{ fontSize: 10, color: 'var(--text4)', fontVariantNumeric: 'tabular-nums' }}>{item.meter_id}</span>}
...
const s = {
  ...
  itemActive: { borderColor: '#2563eb', boxShadow: '0 0 0 2px #2563eb33', background: 'var(--surface)' },
