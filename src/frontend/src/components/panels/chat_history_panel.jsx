...
import { MessageSquare, Trash2, Search, X, Bot } from 'lucide-react'
...
export default function ChatHistoryPanel() {
  ...
  return (
    <div style={s.wrap}>
      <div style={s.header}>
        <div>
          <div style={s.title}><MessageSquare size={14} style={{verticalAlign:'-2px',marginRight:5}}/>AI 대화 내역</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            ...
            style={s.clearAllBtn}
            title="전체 대화 기록 삭제"
><Trash2 size={12} style={{verticalAlign:'-1px',marginRight:4}}/>전체 삭제</button>
        </div>
      </div>
        ...
        <div style={s.viewer}>
          {!selectedId && (
            <div style={s.viewerEmpty}>
              <div style={{ fontSize: 36, marginBottom: 12 }}><MessageSquare size={36} strokeWidth={1.2}/></div>
              ...
              <div style={s.messages}>
                {(detail.messages ?? []).map((m, i) => (
                  <div key={i} style={{ ...s.row, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    {m.role !== 'user' && <div style={s.avatar}><Bot size={14}/></div>}
