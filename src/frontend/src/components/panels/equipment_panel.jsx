...
import { Factory, RefreshCw, TrendingUp, Loader2, Lightbulb, X, Zap, Bot, Wrench, Check, Settings } from 'lucide-react'
import { EquipmentIcon } from '../common/equipment_icon'
import { getEquipmentStatus, diagnoseEquipment, createWorkOrder, getPredictiveMaintenance, prewarmDiagnoses } from '../../api/client'
          ...
          return (
            <div key={it.id} style={s.predRow}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {it.indicator_label && (
                  <div style={{ fontSize: 9, color: 'var(--text4)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 2 }}>{it.indicator_label}</div>
                )}
                <span style={{ fontSize: 11, color: it.projection ? '#f0b429' : 'var(--text3)' }}>{it.note}</span>
              </div>
            </div>
          )
        })}
  ...
  return (
    ...
    >
      ...
      <div style={s.metricRow}>
        ...
        <span style={{ ...s.metricValue, color: st.color }}>
...
export default function EquipmentPanel({ onEquipmentClick, onWorkOrderCreated, onFocusEquipment, onNavigate, onAskAI } = {}) {
  const [items,   setItems]   = useState([])
  const [sortAsc,  setSortAsc]  = useState(true)   // true=경고→정상, false=정상→경고
    ...
    <div style={s.wrap}>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => setSortAsc(v => !v)} style={{ ...s.refreshBtn, fontSize: 11 }}>
            {sortAsc ? '경고→정상' : '정상→경고'}
          </button>
      ...
      <div style={s.body}>
        {loading && (
          <div style={s.center}>
            <div style={{ fontSize: 28, marginBottom: 12 }}><Loader2 size={28} strokeWidth={1.5}/></div>
        ...
        {!loading && !error && (
          <div style={s.grid}>
            {(() => {
              const predMap = predictive.reduce((m, p) => { m[p.id] = p; return m }, {})
              const ORDER = { 경고: 0, 주의: 1, 정상: 2 }
        ...
        {!loading && !error && (
          <div style={s.hint}>
            <Lightbulb size={13} style={{verticalAlign:'-2px',marginRight:5}}/>설비 카드를 클릭하면 AI가 고장 원인과 권장 조치를 진단합니다.
          </div>
        )}
      </div>
...
function DiagnosisModal({ item, diag, loading, onRegen, onClose, onViewAnomalies, onCreateWO, woState, onNavigate, onAskAI }) {
  const st = STATUS_STYLE[item.status] ?? STATUS_STYLE.정상
  const c  = item.counts ?? {}
  return (
    <div style={s.overlay} onClick={onClose}>
      <div style={s.modal} onClick={e => e.stopPropagation()}>
        ...
        {!loading && diag?.electrical_str && (
          <div style={{ ...s.elecBar, flexDirection: 'column', gap: 6 }}>
            <div style={{ fontSize: 10, color: 'var(--text4)', fontWeight: 600, display:'flex', alignItems:'center', gap:3 }}>
              <Zap size={10}/>전기 측정값
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px 16px' }}>
              {diag.electrical_str.split(', ').map((item, i) => {
                const isAbnormal = (/-\d/.test(item)) || (/(\d+\.?\d*)\s*%/.test(item) && parseFloat(item.match(/(\d+\.?\d*)\s*%/)?.[1] ?? 0) > 5)
                return (
                  <span key={i} style={{ fontSize: 11, color: isAbnormal ? '#f85149' : 'var(--text2)', fontWeight: isAbnormal ? 600 : 400 }}>
                    {isAbnormal ? '⚠ ' : ''}{item}
                  </span>
                )
              })}
        ...
        <div style={s.modalBody}>
          {loading && (
            <div style={{ textAlign: 'center', padding: 30, color: 'var(--text3)', fontSize: 13 }}>
              <Bot size={13} style={{verticalAlign:'-2px',marginRight:5}}/>AI가 데이터를 분석하고 있습니다...
            </div>
          )}
          {!loading && diag && (
            <>
              ...
              <div style={s.llmTag}>
                {diag.llm_used ? <><Bot size={11} style={{verticalAlign:'-1px',marginRight:4}}/>AI 진단</> : <><Settings size={11} style={{verticalAlign:'-1px',marginRight:4}}/>규칙 기반 요약 (LLM 미사용)</>}
              </div>
            </>
          )}
        </div>
        ...
        <div style={s.modalFooter}>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onRegen} disabled={loading} style={s.ghostBtn}><RefreshCw size={11} style={{verticalAlign:'-1px',marginRight:4}}/>다시 진단</button>
            {onAskAI && diag && (
              <button onClick={() => onAskAI(`설비 "${item.name}" 진단 결과 (헬스 ${item.health_score}/100, 상태: ${item.status}):
${(diag.diagnosis ?? '').slice(0, 300)}

이 설비의 조치 방법과 유지보수 계획을 자세히 알려줘.`)} style={{ ...s.ghostBtn, color: '#a371f7', borderColor: '#a371f755' }}>
                <Bot size={11} style={{verticalAlign:'-1px',marginRight:4}}/>AI에게 물어보기
              </button>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {woState === 'saving' ? '생성 중...' : woState === 'done' ? <><Check size={11} style={{verticalAlign:'-1px',marginRight:4}}/>작업지시 생성됨</> : <><Wrench size={11} style={{verticalAlign:'-1px',marginRight:4}}/>작업지시 생성</>}
            </button>
            {woState === 'done' && (
              <button onClick={() => { onClose(); onNavigate?.('maintenance') }} style={s.navBtn}>
                정비 패널에서 확인 →
              </button>
            )}
          </div>
...
const s = {
  ...
  elecBar:  { margin: '0 20px', padding: '8px 12px', background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 4 },
  navBtn:   { padding: '8px 14px', background: '#2563eb22', border: '1px solid #2563eb66', borderRadius: 6, color: '#2563eb', fontSize: 12, cursor: 'pointer', fontWeight: 700 },
  modalBody: { padding: '12px 20px 16px', overflowY: 'auto', flex: 1 },
  llmTag:   { marginTop: 12, fontSize: 10, color: 'var(--text4)', textAlign: 'right' },
  modalFooter: { padding: '12px 20px', borderTop: '1px solid var(--line)', background: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: 8 },
