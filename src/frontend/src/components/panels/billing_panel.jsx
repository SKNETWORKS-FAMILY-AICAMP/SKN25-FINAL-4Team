...
import { Wallet, TrendingUp, RefreshCw, Bot, Lightbulb, Settings } from 'lucide-react'
import {
  LineChart, Line, BarChart, Bar, XAxis, YAxis, Tooltip,
  ResponsiveContainer, CartesianGrid, ReferenceLine,
} from 'recharts'
import { getBilling, getPeakForecast, getSettings, updateSettings } from '../../api/client'

const fmtEur = v => (v == null) ? '–' : '€ ' + Math.round(v).toLocaleString('ko-KR')
...
export default function BillingPanel() {
  ...
  const [month,   setMonth]   = useState(null)  // null = 백엔드 자동 결정
  ...
  const load = (m) => {
    ...
    getBilling(m ?? month, unitPrice, null, target)
      .then(r => {
        if (r.data.error) throw new Error(r.data.error)
        setData(r.data)
        if (r.data.period && !m && !month) setMonth(r.data.period)
      })
      .catch(e => setError(e.message ?? '로드 실패'))
      .finally(() => setLoading(false))
  }

  const loadPeak = (showLoading = true) => {
    if (showLoading) setPeakLoading(true); setPeakErr(false)
    getPeakForecast()
      .then(r => {
  ...
  // 라이브 데이터 기준으로 피크 예측을 주기적으로 갱신한다.
  useEffect(() => {
    const id = setInterval(() => loadPeak(false), 30000)
    ...
    <div style={s.wrap}>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: 'var(--text4)' }}>조회 월</span>
          <select value={month ?? ''} onChange={e => { const m = e.target.value || null; setMonth(m); load(m) }}
            style={{ ...s.input, width: 90, cursor: 'pointer' }}>
            {['2023-01','2023-02','2023-03','2023-04','2023-05','2023-06',
              '2023-07','2023-08','2023-09','2023-10','2023-11'].map(m => (
              <option key={m} value={m}>{m}</option>
            ))}
          </select>
          <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--text4)' }}>단가</span>
        ...
        <div style={s.body}>
          ...
          {data.narrative && (
            <div style={s.narrativeBox}>
              <div style={{...s.narrativeTitle, display:'flex', alignItems:'center', gap:5}}><Bot size={12}/>AI 비용 분석 · 권고</div>
            ...
            <div style={s.chartBox}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                ...
                >
                  ...
                  <Tooltip {...tt} formatter={v => [`€ ${Math.round(v).toLocaleString('ko-KR')}`, '누적']}/>
          ...
          <div style={s.infoBox}>
            <span style={{ fontSize: 11, color: 'var(--text4)' }}>
              <Lightbulb size={11} style={{verticalAlign:'-1px',marginRight:4}}/>현재 독일 산업용 전기요금({unitPrice} €/kWh) 기준 ·
              한국 적용 시 <strong style={{ color: '#2563eb' }}><Settings size={10} style={{verticalAlign:'-1px',marginRight:2}}/>시스템 설정</strong>에서 요금표 교체 가능
            </span>
          </div>
        </div>
      )}
