...
import { Wrench, ClipboardList, Check, Play, RefreshCw, Inbox, Bot, ChevronDown, ChevronUp, Factory } from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import { listWorkOrders, updateWorkOrderStatus } from '../../api/client'

const STATUS = {
  open:        { label: '열림',   color: '#2563eb', Icon: ClipboardList },
  in_progress: { label: '진행중', color: '#d29922', Icon: Wrench },
  done:        { label: '완료',   color: '#3fb950', Icon: Check },
}
const PRIORITY = {
...
function WorkOrderCard({ wo, busy, onAdvance, onComplete, onNavigateEquipment, onAskAI }) {
  ...
  const handleCompleteSubmit = () => {
    onComplete(wo, note.trim() || '완료 처리됨')
    setCompleting(false)
    setNote('')
  }
    ...
    <div style={s.card}>
      <div style={s.cardHead}>
        <div style={{ minWidth: 0 }}>
          <div
            style={{ ...s.eq, cursor: onNavigateEquipment ? 'pointer' : 'default', display: 'inline-flex', alignItems: 'center', gap: 4 }}
            onClick={() => onNavigateEquipment?.(wo.equipment_id)}
          >
            {onNavigateEquipment && <Factory size={10} style={{ opacity: 0.6 }} />}
      ...
      {wo.action && (
        <div style={{ marginTop: 8 }}>
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: 11, padding: '2px 0', display: 'flex', alignItems: 'center', gap: 4 }}
          >
            {showAction ? <ChevronUp size={12}/> : <ChevronDown size={12}/>}
            AI 진단 상세 {showAction ? '접기' : '보기'}
          </button>
          {showAction && (
      ...
      {wo.status === 'done' && wo.outcome_note && (
        <div style={s.outcome}><Check size={11} style={{verticalAlign:'-1px',marginRight:4}}/>{wo.outcome_note}</div>
      )}
          ...
          {completing ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <textarea
                ...
                placeholder="조치 결과를 기록하세요..."
                rows={3}
                style={{ width: '100%', fontSize: 11, padding: '6px 8px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', resize: 'vertical', boxSizing: 'border-box' }}
                autoFocus
              />
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={handleCompleteSubmit} disabled={busy} style={s.doneBtn}><Check size={11} style={{verticalAlign:'-1px',marginRight:4}}/>완료 확인</button>
                <button onClick={() => { setCompleting(false); setNote('') }} style={s.cancelBtn}>취소</button>
              </div>
            </div>
          ) : (
            <div style={s.actions}>
              {wo.status === 'open' && (
                <button disabled={busy} onClick={() => onAdvance(wo, 'in_progress')} style={s.startBtn}><Play size={11} style={{verticalAlign:'-1px',marginRight:4}}/>작업 시작</button>
              )}
              {wo.status === 'in_progress' && (
                <button disabled={busy} onClick={() => setCompleting(true)} style={s.doneBtn}><Check size={11} style={{verticalAlign:'-1px',marginRight:4}}/>완료 처리</button>
              )}
              {onAskAI && (
                <button
                  onClick={() => onAskAI(`설비 "${wo.equipment_name}" 작업지시: ${wo.title}\n원인: ${wo.cause}\n\n이 정비 작업의 구체적인 절차와 주의사항을 알려줘.`)}
                  style={{ ...s.cancelBtn, color: '#a371f7', borderColor: '#a371f755' }}
                >
                  <Bot size={11} style={{verticalAlign:'-1px',marginRight:4}}/>AI 문의
                </button>
              )}
            </div>
          )}
...
export default function MaintenancePanel({ refreshSignal, onNavigate, onAskAI } = {}) {
  ...
  const complete = (wo, note) => advance(wo, 'done', note)

  const navigateEquipment = onNavigate ? (equipmentId) => {
    ...
    <div style={s.wrap}>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={load} style={s.refreshBtn} disabled={loading}><RefreshCw size={11} style={{verticalAlign:'-1px',marginRight:4}}/>새로고침</button>
        </div>
      </div>
      ...
      <div style={s.body}>
        ...
        {!loading && !error && items.length === 0 && (
          <div style={s.center}>
            <div style={{ marginBottom: 10 }}><Inbox size={32} strokeWidth={1.2}/></div>
            <div style={{ fontSize: 14, color: 'var(--text)', fontWeight: 600 }}>작업지시가 없습니다</div>
            <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 6 }}>
              설비 상태 감시 → 카드 클릭 → AI 진단에서 "작업지시 생성"으로 추가하세요.
            </div>
            {onNavigate && (
              <button onClick={() => onNavigate('equipment')} style={{ ...s.refreshBtn, marginTop: 14, fontSize: 12 }}>
                <Factory size={12} style={{verticalAlign:'-1px',marginRight:4}}/>설비 상태 감시로 이동
              </button>
            )}
          </div>
        )}
        {!loading && !error && items.length > 0 && (
          <div style={s.columns}>
            {groups.map(({ st, list }) => {
              const { Icon, label, color } = STATUS[st]
              return (
                <div key={st} style={s.column}>
                  <div style={{ ...s.colHead, color }}>
                    <Icon size={13} style={{verticalAlign:'-1px',marginRight:5}}/>{label}
                    <span style={{ color: 'var(--text4)', marginLeft: 4 }}>({list.length})</span>
                  </div>
                  <div style={s.colBody}>
                    {list.length === 0
                      ...
                      : list.map(wo => (
                          <WorkOrderCard
                            ...
                            onAdvance={advance}
                            onComplete={complete}
                            onNavigateEquipment={navigateEquipment}
                            onAskAI={onAskAI}
                          />
                        ))}
                  </div>
...
const s = {
  ...
  refreshBtn: { padding: '7px 14px', background: 'var(--line)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text)', fontSize: 12, cursor: 'pointer', fontWeight: 600, display: 'inline-flex', alignItems: 'center' },
  ...
  colHead: { padding: '10px 14px', fontSize: 13, fontWeight: 700, borderBottom: '1px solid var(--line)', background: 'var(--surface)', flexShrink: 0, display: 'flex', alignItems: 'center' },
  ...
  actionBox: { marginTop: 6, background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: 6, padding: '6px 10px' },
  ...
  startBtn:{ flex: 1, padding: '7px 10px', background: '#2563eb', border: 'none', borderRadius: 6, color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  doneBtn: { flex: 1, padding: '7px 10px', background: '#238636', border: 'none', borderRadius: 6, color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  cancelBtn: { padding: '7px 10px', background: 'none', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text3)', fontSize: 12, cursor: 'pointer', display: 'inline-flex', alignItems: 'center' },
}
