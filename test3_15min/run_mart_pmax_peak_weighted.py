...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "peak_weighted_outputs"
WEATHER_METER_URN = "WeatherStation.Weather"

STEP_MINUTES = 15
INPUT_WINDOW_HOURS = [24]
OUTPUT_HORIZON_STEPS = {"15min": 1}
...
def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched["is_business_hour"] = ((minute_of_day >= 8 * 60) & (minute_of_day < 18 * 60)).astype(float)
    enriched["is_morning_ramp"] = ((minute_of_day >= 6 * 60) & (minute_of_day < 10 * 60)).astype(float)
    enriched["is_lunch_time"] = ((minute_of_day >= 11 * 60) & (minute_of_day < 14 * 60)).astype(float)
    enriched["is_evening"] = ((minute_of_day >= 17 * 60) & (minute_of_day < 22 * 60)).astype(float)
    return enriched


def add_segment_features(segment: pd.DataFrame) -> pd.DataFrame:
    enriched = segment.sort_values("ts").copy()
    p_max = enriched["P_max"]
    for lag in [1, 2, 4, 8, 96, 192]:
        enriched[f"P_max_lag_{lag}"] = p_max.shift(lag)
    for steps, name in [(4, "1h"), (12, "3h"), (24, "6h")]:
        rolling = p_max.rolling(steps, min_periods=1)
        enriched[f"P_max_roll_{name}_mean"] = rolling.mean()
        enriched[f"P_max_roll_{name}_max"] = rolling.max()
    enriched["P_max_roll_1h_std"] = p_max.rolling(4, min_periods=2).std()
    enriched["P_max_roll_24h_max"] = p_max.rolling(96, min_periods=1).max()
    enriched["P_max_diff_1"] = p_max.diff(1)
    enriched["P_max_diff_4"] = p_max.diff(4)
    enriched["P_mean_diff_1"] = enriched["P_mean"].diff(1)
    enriched["U1_mean_diff_1"] = enriched["U1_mean"].diff(1)
    enriched["PF_mean_diff_1"] = enriched["PF_mean"].diff(1)
    return enriched
...
def add_lag_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, segment in df.groupby(["source_meter_urn", "segment_id"], sort=True):
        frames.append(add_segment_features(segment))
...
def fetch_logical_meter(engine, table_name: str, logical_meter: str, weather: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched = add_time_features(combined.sort_values(["segment_id", "ts"]).reset_index(drop=True))
    return add_lag_rolling_features(enriched).sort_values(["segment_id", "ts"]).reset_index(drop=True)
...
def peak_sample_weights(reference_y: np.ndarray, fit_y: np.ndarray) -> np.ndarray:
    reference = reference_y.reshape(-1)
    fit_target = fit_y.max(axis=1)
    q90 = float(np.nanquantile(reference, 0.90))
    q95 = float(np.nanquantile(reference, 0.95))
    weights = np.ones(len(fit_y), dtype=np.float32)
    weights[fit_target >= q90] = PEAK_WEIGHT_Q90
    weights[fit_target >= q95] = PEAK_WEIGHT_Q95
    return weights


def train_v23(
    bundle: DatasetBundle, seed: int, x_fit: np.ndarray, y_fit: np.ndarray, sample_weight: np.ndarray
) -> list[LGBMRegressor]:
    models = []
...
CANDIDATE_VERSIONS = ["v20", "v23", "v25", "v27"]
...
def train_candidates(bundle: DatasetBundle, model_dir: Path, seed: int, device: str) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    sample_weight = peak_sample_weights(bundle.y_train, y_fit)
    models: dict[str, Any] = {}
    rows = []
    factories = {
    ...
    v23 = train_v23(bundle, seed, x_fit, y_fit, sample_weight)
    models["v23"] = v23
...
def run_one(
    ...
    manifest_common = {
        ...
        "sample_weight_policy": {
            "base": 1.0,
            "target_q90": PEAK_WEIGHT_Q90,
            "target_q95": PEAK_WEIGHT_Q95,
        },
...
def main() -> None:
    ...
    output_dir = args.output_dir or (
        SCRIPT_DIR / ("peak_weighted_outputs_gpu" if args.device == "gpu" else "peak_weighted_outputs")
    )
    output_dir.mkdir(parents=True, exist_ok=True)
