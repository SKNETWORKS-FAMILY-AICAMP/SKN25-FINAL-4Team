...
print('db_env_present|' + '|'.join(f'{k}={bool(env.get(k))}' for k in ['DB_HOST','DB_PORT','DB_NAME','DB_USER','DB_PASSWORD']))
conn = psycopg.connect(host=env['DB_HOST'], port=int(env.get('DB_PORT','5432')), dbname=env['DB_NAME'], user=env['DB_USER'], password=env['DB_PASSWORD'], connect_timeout=10)
