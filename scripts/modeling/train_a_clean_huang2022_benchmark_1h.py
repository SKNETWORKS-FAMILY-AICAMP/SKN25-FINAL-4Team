#!/usr/bin/env python3
"""Huang et al. 2022-style benchmark adaptation for EMS A-clean 1h targets.

Paper in scope, per user-provided summary:
Energy Forecasting in a Public Building: A Benchmarking Analysis on Long Short-Term
Memory (LSTM), Support Vector Regression (SVR), and Extreme Gradient Boosting
(XGBoost) Networks, Applied Sciences, Huang et al. 2022.

Adaptation boundary:
- Paper data resolution: 30 min public-building electricity consumption.
- EMS cache resolution here: 1 h A-clean electricity-consumption targets.
- Paper-style input policy: historical consumption + time-of-day sine/cosine only.
- Excluded in this script: weather, rolling statistics, load-bin/event features,
  target aggregates outside the A-clean 4-target set.
"""
...
def make_frame(target: pd.DataFrame, feature: pd.DataFrame, target_id: str, lookback: int) -> pd.DataFrame:
    ...
    # Input-only fallback for early rows; rows with full lookback are used for learning/evaluation.
...
def fit_xgboost_candidate(
    ...
    train_mask = usable_mask(df) & (df["split"] == "train") & (~df["is_gateway_outage"].fillna(False))
    val_mask = usable_mask(df) & (df["split"] == "validation") & (~df["is_gateway_outage"].fillna(False))
...
def make_prediction_frame(df: pd.DataFrame, pred_cols: list[str], prediction: np.ndarray, candidate_col: str, candidate_name: str) -> pd.DataFrame:
    ...
    pred_df["usable_non_gateway"] = (metric_mask & (~df["is_gateway_outage"].fillna(False))).to_numpy(dtype=bool)
    return pred_df
...
def fit_lstm_candidate(df: pd.DataFrame, lookback: int, hidden_size: int, epochs: int, patience: int, target_id: str, seed: int) -> CandidateResult:
    ...
    y_mean = float(np.mean(train_y)); y_std = float(np.std(train_y) or 1.0)
...
def run_target(
    ...
    effective_lstm_epochs = int(lstm_epochs if lstm_epochs is not None else (12 if quick else 35))
    effective_lstm_patience = int(lstm_patience if lstm_patience is not None else (3 if quick else 10))

    svr_grid = [(10.0, 0.1), (100.0, 0.1)] if quick else [(1.0, 0.1), (10.0, 0.1), (100.0, 0.1), (10.0, 0.01)]
    for C, eps in svr_grid:
        results.append(fit_sklearn_candidate(
            df, tabular_cols,
            Pipeline([("scale", StandardScaler()), ("model", SVR(kernel="rbf", C=C, epsilon=eps, gamma="scale", cache_size=1000))]),
            f"svr_rbf_C{C:g}_eps{eps:g}", "SVR", target_id, max_train_rows=8000,
        ))
    ...
    for res in results:
        ...
        print(json.dumps({
            ...
            "test_non_gateway_mae": res.row.get("test_non_gateway_mae"),
    ...
    metrics.to_csv(target_out / "huang2022_candidate_metrics.csv", index=False)
    ...
    pred_df = make_prediction_frame(df, pred_cols, best_res.prediction, candidate_col="benchmark_candidate", candidate_name=best_name)
    pred_df.to_parquet(target_out / "huang2022_best_predictions.parquet", index=False)
    if best_res.artifact is not None:
        if best["model_family"] == "LSTM":
            torch.save(best_res.artifact, target_out / "huang2022_best_model.pt")
        else:
            joblib.dump(best_res.artifact, target_out / "huang2022_best_model.joblib", compress=3)
    with (target_out / "huang2022_manifest.json").open("w", encoding="utf-8") as f:
        json.dump({
            "created_at_utc": utc_now(),
            "target_id": target_id,
            "paper_adaptation": "Huang et al. 2022-style: historical consumption + hour_sin/hour_cos only; 1h EMS adaptation of 30min paper.",
            ...
            "prediction_parquet_policy": "Saved prediction parquet keeps the full target split rows and adds usable_for_metric plus usable_non_gateway flags for plot/report filtering.",
            "best": best,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=Path("outputs/modeling/a_clean_huang2022_benchmark_1h"))
    p.add_argument("--target-id", action="append", default=None)
    p.add_argument("--lookback", type=int, default=24)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--quick", action="store_true")
    p.add_argument("--lstm-epochs", type=int, default=None, help="Override LSTM epochs. Default: 12 quick, 35 normal.")
...
def main() -> None:
    ...
    best_rows = [run_target(target, feature, target_id, out_dir, args.lookback, args.seed, args.quick, args.lstm_epochs, args.lstm_patience) for target_id in targets]
    best_df = pd.DataFrame(best_rows)
    best_df.to_csv(out_dir / "huang2022_summary.csv", index=False)
    effective_lstm_epochs = int(args.lstm_epochs if args.lstm_epochs is not None else (12 if args.quick else 35))
    effective_lstm_patience = int(args.lstm_patience if args.lstm_patience is not None else (3 if args.quick else 10))
    manifest = {
        "created_at_utc": utc_now(),
        "run_label": "a_clean_huang2022_benchmark_1h",
        "paper": {
            "title": "Energy Forecasting in a Public Building: A Benchmarking Analysis on Long Short-Term Memory (LSTM), Support Vector Regression (SVR), and Extreme Gradient Boosting (XGBoost) Networks",
            "doi": "10.3390/app12199788",
            "source": "Applied Sciences",
            "year": 2022,
        },
        "adaptation_boundary": "Paper is 30min; this run uses EMS A-clean 1h cache. Inputs limited to historical target consumption and hour_sin/hour_cos.",
