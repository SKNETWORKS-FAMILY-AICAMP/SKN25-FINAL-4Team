...
The script keeps the canonical source in ``docs/specs/diagrams/*.mmd`` and
...
DIAGRAM_DIR = ROOT / "docs/specs/diagrams"
CONFIG_PATH = DIAGRAM_DIR / "mermaid_render_config.json"
...
def render_png_preview(svg_path: Path) -> Path:
    ...
    target = PNG_PREVIEW_DIR / svg_path.with_suffix(".png").name
    subprocess.run(
