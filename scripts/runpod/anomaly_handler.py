...
PROJECT_ROOT = Path(os.getenv("CMS_PROJECT_ROOT", str(Path(__file__).resolve().parents[2]))).resolve()
ARTIFACTS_DIR = Path(os.getenv("ANOMALY_ARTIFACTS_DIR", str(PROJECT_ROOT / "artifacts" / "anomaly"))).resolve()
CANDIDATE_DIR = Path(os.getenv("ANOMALY_CANDIDATE_DIR", str(ARTIFACTS_DIR.parent / "anomaly_candidates")))
ARCHIVE_STAGING_DIR = Path(os.getenv("RUNPOD_ARCHIVE_STAGING_DIR", str(ARTIFACTS_DIR.parent / "anomaly_incoming")))
DEFAULT_LOG_DIR = Path(os.getenv("RUNPOD_JOB_LOG_DIR", "/tmp/runpod_anomaly_job_logs"))
...
def _read_input(job: dict[str, Any]) -> dict[str, Any]:
    ...
    model_kind = raw.get("model_kind", "anomaly")
    if model_kind != "anomaly":
        raise JobInputError("this handler only supports model_kind=anomaly")
    ...
    result = {
        ...
        "save_diagnostics": _as_bool(raw.get("save_diagnostics"), "save_diagnostics", default=False),
...
def _download_training_data(req: dict[str, Any], log_dir: Path) -> Path | None:
    ...
    try:
        ...
        conn.putheader("User-Agent", "cms-anomaly-runpod-job/1.0")
...
def _assert_candidate_layout(candidate_dir: Path, horizon: int) -> int:
    ...
    meter_count = len([path for path in horizon_dir.iterdir() if path.is_dir()])
    if meter_count < 1:
...
def _upload_archive_once(req: dict[str, Any], archive_path: Path) -> dict[str, Any]:
    ...
    boundary = f"----runpod-anomaly-artifact-{uuid.uuid4().hex}"
...
def _main() -> None:
    parser = argparse.ArgumentParser(description="RunPod Serverless anomaly retraining local runner")
