...
OUT_PATH = Path.home() / "SKN25-FINAL-4Team/outputs/aihub_v84_fullretrain_v2_per_device.csv"
...
# ── preprocessing.py 분할 함수 패치 (val/test 각 7일 고정) ───────────────────
...
def _patched_determine_split_bounds(frame):
    ...
    min_ts = frame["ts"].min()
    max_ts = frame["ts"].max()
    if min_ts <= TRAIN_START and max_ts >= END_TS - _pd.Timedelta(hours=1):
        return _original_determine_split_bounds(frame)
    start = min_ts.ceil("h")
    end   = (max_ts + _pd.Timedelta(hours=1)).floor("h")
    test_start = end  - _pd.Timedelta(days=7)
    val_start  = test_start - _pd.Timedelta(days=7)
    if val_start <= start:
        return _original_determine_split_bounds(frame)
    return {"train_start": start, "val_start": val_start,
            "test_start": test_start, "end": end}
