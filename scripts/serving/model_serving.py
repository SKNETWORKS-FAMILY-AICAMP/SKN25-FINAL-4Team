...
def main() -> int:
    ...
    parser.add_argument("--pmax-artifact-root", help="Extracted P-Max release root")
    parser.add_argument("--skip-pmax", action="store_true", help="Run anomaly-only serving without loading P-Max artifacts")
    ...
    if args.enable_anomaly_db_read and not args.anomaly_artifact_root:
        return _finish(
            {
                "ok": False,
                "blocked": True,
                "error": "--enable-anomaly-db-read requires --anomaly-artifact-root",
                "write_attempted": False,
            },
            json_only=args.json,
            exit_code=2,
        )
    if not args.skip_pmax and not args.pmax_artifact_root:
        return _finish(
            {
                "ok": False,
                "blocked": True,
                "error": "--pmax-artifact-root is required unless --skip-pmax is set",
                "write_attempted": False,
            },
            json_only=args.json,
            exit_code=2,
    ...
    if not args.skip_pmax:
        ...
        if not pmax_features.ok and (not args.allow_harmonized_observed_input or not pmax_features.features):
            ...
            raw_errors = pmax_features.errors or issue_reasons
            compact_errors = _compact_errors(raw_errors)
            pmax_error_count = len(raw_errors)
            finished_at = datetime.now(tz=UTC)
            failure_write = _write_pmax_failure_log(
                config=config,
                run_id=run_id,
                job_id=args.job_id,
                base_ts=base_ts,
                ...
                artifact_ref=str(Path(args.pmax_artifact_root)) if args.pmax_artifact_root else "",
                stage="pmax_feature_build",
                errors=compact_errors,
                pmax_input_rows=pmax_input_rows,
                pmax_error_count=pmax_error_count,
            ...
            return _finish(
                {
                    ...
                    "pmax_readiness_issue_count": readiness_issue_count,
                    "pmax_harmonized_inferred_rows": harmonized_inferred_rows,
                    "write_attempted": failure_write["attempted"],
                    "written_tables": failure_write["written_tables"],
                    "written_rows": failure_write["written_rows"],
                    "write_errors": failure_write["errors"],
                },
                json_only=args.json,
                exit_code=2,
            )
    ...
    if args.anomaly_predictions_path:
        ...
        try:
            anomaly_payload = _predict_anomaly_feature_rows(
                rows=anomaly_read.rows,
                forecast_origin_ts=base_ts,
                artifact_root=Path(args.anomaly_artifact_root),
            )
            anomaly_result = AnomalyWarningAdapter(model=_StaticAnomalyPredictor(anomaly_payload)).predict(
                ({"forecast_origin_ts": base_ts, "source_mode": SOURCE_MODE_LIVE_OBSERVED},)
            )
            anomaly_rows = anomaly_result.long_rows
            anomaly_disabled_reason = None
            anomaly_source_mode = SOURCE_MODE_LIVE_OBSERVED
            anomaly_source_table = anomaly_query.source_tables[0]
        except Exception as exc:  # noqa: BLE001 - live anomaly branch should report a compact operational blocker.
            if not args.allow_missing_anomaly:
                return _finish(
                    {
                        "ok": False,
                        "stage": "anomaly_feature_predict",
                        "errors": [str(exc)],
            ...
            anomaly_disabled_reason = f"anomaly live feature branch failed: {exc}"
            anomaly_source_mode = SOURCE_MODE_LIVE_OBSERVED
            anomaly_source_table = anomaly_query.source_tables[0]
    ...
    evidence_packet = {
        ...
        "pmax_input_rows": pmax_input_rows,
        "pmax_feature_count": pmax_feature_count,
    ...
    batch = build_model_serving_write_batch(
        ...
        started_at=started_at,
        finished_at=finished_at,
        artifact_refs={"pmax": str(Path(args.pmax_artifact_root)) if args.pmax_artifact_root else "", "anomaly": args.anomaly_predictions_path or args.anomaly_artifact_root or ""},
        pmax_rows=pmax_rows,
    ...
    ok = bool(pmax_ok and (write_result.ok or not args.execute_write))
    return _finish(
        {
            "ok": ok,
            "run_id": run_id,
...
def _write_pmax_failure_log(
    *,
    ...
    errors: Sequence[str],
    pmax_input_rows: int,
    pmax_error_count: int,
    pmax_readiness_issue_count: int,
    pmax_harmonized_inferred_rows: int,
    execute_write: bool,
) -> dict[str, Any]:
    if not execute_write or os.environ.get("ALLOW_MODEL_SERVING_WRITE") != "1":
        return {"attempted": False, "written_tables": [], "written_rows": 0, "errors": []}
    ...
    payload = {
        ...
        "internal_missing_segment_count": pmax_readiness_issue_count,
        "latest_missing_policy": None,
        "error_reason": "missing_required_measurements",
        "details": {
            "job_id": job_id,
            ...
            "pmax_readiness_issue_count": pmax_readiness_issue_count,
            "pmax_harmonized_inferred_rows": pmax_harmonized_inferred_rows,
        },
        "started_at": started_at,
        "completed_at": finished_at,
    }
    try:
        PsycopgModelServingSink(config).write_rows(schema="ops", table="pmax_log", rows=(payload,))
    except Exception as exc:  # noqa: BLE001 - preserve the original readiness failure response.
        return {"attempted": True, "written_tables": [], "written_rows": 0, "errors": [type(exc).__name__]}
    return {"attempted": True, "written_tables": ["ops.pmax_log"], "written_rows": 1, "errors": []}
...
def _predict_anomaly_feature_rows(*, rows: Any, forecast_origin_ts: datetime, artifact_root: Path) -> tuple[dict[str, Any], ...]:
    """Run anomaly artifacts on mart.anomaly_feature_1h rows without writing results."""
    ...
    materialized = tuple(dict(row) for row in rows)
    if not materialized:
        raise ValueError("live anomaly feature query returned no rows")
    frame = pd.DataFrame(materialized)
    required = {"bucket_ts", "meter_urn", "p_value"}
    missing = sorted(required.difference(frame.columns))
    if missing:
        raise ValueError("live anomaly feature rows missing columns: " + ",".join(missing))
    ...
    for meter_urn, meter_frame in frame.groupby("meter_urn", sort=True):
        model_urn = anomaly_model_urn_for_meter(str(meter_urn))
        wide = meter_frame.sort_values("ts")[[col for col in ("ts", "P", "U1", "PF", "qv", "Tdiff") if col in meter_frame.columns]]
        result = predictor.predict_meter(None, str(meter_urn), model_urn, 3, ts, raw_data=wide)
        if not result:
            continue
        result = dict(result)
        result["forecast_origin_ts"] = forecast_origin_ts.isoformat()
        result["created_at"] = forecast_origin_ts.isoformat()
        result["source_mode"] = SOURCE_MODE_LIVE_OBSERVED
        result["source_input_refs"] = (
            f"mart.anomaly_feature_1h:{meter_urn}:{meter_frame['ts'].min()}:{meter_frame['ts'].max()}:source_mode={SOURCE_MODE_LIVE_OBSERVED}",
        )
        result["reference_input_row_count"] = int(len(meter_frame))
        result["reference_wide_row_count"] = int(len(wide))
        predictions.append(result)
    if not predictions:
        raise ValueError("live anomaly feature artifact produced no predictions")
    return tuple(predictions)
