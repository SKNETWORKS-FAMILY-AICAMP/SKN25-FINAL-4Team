...
from scripts.eda.correlation.eda_raw_correlation_representative_electric import fetch_meter_raw_df
...
def collect_meter_payload(engine, meter_urn: str, output_dir: Path) -> dict[str, Any]:
    try:
        df, measurements = fetch_meter_raw_df(engine, meter_urn)
    except Exception as exc:
        print(f"{meter_urn} 데이터 로드 실패: {exc}")
        return {"label": meter_urn, "description": "", "group": "", "energy_type": "", "features": []}
...
def generate_dashboard(
    ...
    for meter_urn in meters:
        meter_manifest = collect_meter_payload(engine, meter_urn, output_dir)
