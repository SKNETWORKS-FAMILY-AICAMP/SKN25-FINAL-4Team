...
from scripts.eda.stl.eda_stl_electric import meter_config as electric_meter_config
...
def load_electric_meter_data(engine, weather_df: pd.DataFrame, meter_urn: str) -> pd.DataFrame:
...
def resolve_feature_columns(df: pd.DataFrame, configured_features: list[str]) -> list[str]:
    ...
    for feature in configured_features:
        ...
        if "PF" in df.columns and not df["PF"].isnull().all():
            resolved.append("PF")
            continue

        for fallback in ["PF1", "PF2", "PF3"]:
            if fallback in df.columns and not df[fallback].isnull().all():
