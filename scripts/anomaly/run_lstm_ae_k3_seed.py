...
def main():
    ...
    with mlflow.start_run(run_name="lstm_ae_anomaly_detection"):
        ...
        import joblib, json
        joblib.dump(scaler, "outputs/anomaly/lstm_ae_scaler.pkl")
        with open("outputs/anomaly/lstm_ae_meta.json", "w") as f:
            json.dump({"feat_cols": feat_cols, "threshold_k6": float(train_errors.mean() + 6 * train_errors.std()), "timestep": TIMESTEP}, f)
        print("스케일러 및 메타 저장 완료")
