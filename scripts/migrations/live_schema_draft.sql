...
CREATE TABLE IF NOT EXISTS qa.live_issue (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT live_issue_severity_check CHECK (severity IN ('low', 'medium', 'high', 'block'))
);

CREATE INDEX IF NOT EXISTS live_issue_lookup_idx
    ON qa.live_issue (meter_urn, measurement, created_at);
...
CREATE TABLE IF NOT EXISTS ops.latency (
...
CREATE TABLE IF NOT EXISTS ops.consumer_lag (
...
CREATE TABLE IF NOT EXISTS ops.ingest_metric (
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS pipeline_latency_stage_idx
    ON ops.latency (stage, created_at);

CREATE INDEX IF NOT EXISTS pipeline_latency_bucket_idx
    ON ops.latency (bucket_ts, meter_urn, measurement);

CREATE INDEX IF NOT EXISTS kafka_consumer_lag_lookup_idx
    ON ops.consumer_lag (consumer_group, topic, observed_at);

CREATE INDEX IF NOT EXISTS fastapi_ingest_metric_lookup_idx
    ON ops.ingest_metric (metric_name, created_at);
...
CREATE TABLE IF NOT EXISTS ops.pmax_log (
...
CREATE TABLE IF NOT EXISTS qa.pmax_eval (
    ...
    CONSTRAINT pmax_eval_horizon_check CHECK (horizon_minutes IN (15, 30, 45, 60)),
    CONSTRAINT pmax_eval_values_check CHECK (predicted_p_max >= 0 AND actual_p_max >= 0),
    CONSTRAINT pmax_eval_actual_window_ts_check CHECK (actual_window_ts = target_ts - interval '15 minutes')
);
...
BEGIN
    ...
    IF policy_count = 0 THEN
        ...
        IF disabled_policy_count > 0 THEN
            INSERT INTO qa.live_issue (issue_kind, severity, meter_urn, measurement, event_id, reason)
