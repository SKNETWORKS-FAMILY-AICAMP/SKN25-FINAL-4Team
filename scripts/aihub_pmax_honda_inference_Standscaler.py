"""
Honda v29 가중치 그대로 AI Hub Training 데이터에 추론 (StandardScaler 정규화)
- AI Hub Training 데이터 전체로 StandardScaler fit
...
OUT_PATH = Path.home() / "SKN25-FINAL-4Team/outputs/aihub_pmax_honda_inference_Standscaler_per_device.csv"
...
# ── StandardScaler fit (전체 Training 데이터 기준) ────────────────────────────
print("StandardScaler fit 중 (전체 Training 데이터)...")
all_X = []
...
print(f"StandardScaler fit 완료 (총 {len(all_X_concat)}행)")
...
for device_id in tqdm(train_ids, desc="설비별 추론(Standscaler)"):
    ...
    # StandardScaler 적용 — 피처 단위(22개)로 transform 후 다시 flatten
