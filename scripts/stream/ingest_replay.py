...
PC1_SOURCE_ROOT_ENV = "PC1_LIVE_SOURCE_ROOT"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-root",
        default=None,
        help=f"PC1 gzip archive root containing *_harmonized.csv.gz files. May also be set with {PC1_SOURCE_ROOT_ENV}.",
    )
    parser.add_argument("--source-authority", default=SOURCE_AUTHORITY_PC1_ARCHIVE, help="Source authority label; only pc1_archive is valid for live injection.")
    ...
    parser.add_argument("--meter-urn", action="append", default=[], help="Restrict source files to a meter URN; may repeat.")
    parser.add_argument("--measurement", action="append", default=[], help="Restrict source files to a measurement code; may repeat.")
    ...
    parser.add_argument("--end-ts", default=None, help="Optional inclusive source event_ts where the replay window ends.")
    ...
    source_root = args.source_root or os.getenv(PC1_SOURCE_ROOT_ENV)
    if not source_root:
        raise SystemExit(f"--source-root or {PC1_SOURCE_ROOT_ENV} is required; PC1 is the gzip archive authority")
    ...
    if args.start_ts is not None and args.end_ts is not None and parse_timestamp(args.end_ts) < parse_timestamp(args.start_ts):
        raise SystemExit("--end-ts must be greater than or equal to --start-ts")
    try:
        validate_live_injector_source_authority(args.source_root, args.source_authority)
    except ValueError as exc:
...
def run(args: argparse.Namespace, *, sleep_fn: Callable[[float], None] = time.sleep) -> InjectorSummary:
    ...
    files = filter_source_files(files, meter_urns=tuple(args.meter_urn), measurements=tuple(args.measurement))
...
def measurements_for_files(paths: list[Path]) -> set[str]:
    return {meter_measurement_for_file(path)[1] for path in paths}


def filter_source_files(paths: list[Path], *, meter_urns: tuple[str, ...], measurements: tuple[str, ...]) -> list[Path]:
    wanted_meters = {item.strip() for item in meter_urns if item.strip()}
    wanted_measurements = {item.strip() for item in measurements if item.strip()}
    if not wanted_meters and not wanted_measurements:
        return paths
    selected: list[Path] = []
    for path in paths:
        meter_urn, measurement = meter_measurement_for_file(path)
        if wanted_meters and meter_urn not in wanted_meters:
            continue
        if wanted_measurements and measurement not in wanted_measurements:
            continue
...
def meter_urn_for_file(path: Path) -> str:
    return meter_measurement_for_file(path)[0]


def meter_measurement_for_file(path: Path) -> tuple[str, str]:
    try:
    ...
    return parse_meter_measurement(header)
...
def build_payload(row: SourceRow, *, source_system: str, received_at: datetime | None = None) -> dict[str, Any]:
    ...
    base_payload: dict[str, Any] = {
        ...
        "source_authority": SOURCE_AUTHORITY_PC1_ARCHIVE,
