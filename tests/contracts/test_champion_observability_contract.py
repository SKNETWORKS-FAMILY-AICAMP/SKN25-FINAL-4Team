...
REQUIRED_CHAMPION_PANEL_TITLES = {
    ...
    "P-Max prediction freshness",
    "P-Max quality status",
...
def test_champion_model_panel_contracts_cover_required_grafana_views() -> None:
    ...
    assert panels["P-Max prediction freshness"].source_table == "mart.pmax_forecast_15min"
    assert panels["P-Max quality status"].source_table == "ops.pmax_forecast_inference_log"
    assert panels["P-Max evaluation error"].source_table == "qa.pmax_forecast_evaluation"
