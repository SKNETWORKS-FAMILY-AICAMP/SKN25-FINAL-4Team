...
본 문서는 CMS(Condition Monitoring System)의 데이터 QA, 리포트 생성, 채팅 응답 경로를 연결하는 최소 계약(contract)을 정의합니다. 구현체는 본 문서의 필수 항목을 만족해야 하며, 불확실하거나 검증되지 않은 데이터는 모델 입력, 리포트 근거, 사용자 답변에 직접 사용하지 않아야 합니다.
