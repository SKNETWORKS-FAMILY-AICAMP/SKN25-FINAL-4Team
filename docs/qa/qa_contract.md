...
| `QA-PEAK-001` | Peak/canonical separation | peak/P-Max feature row는 `mart.peak_feature_15min`을 direct runtime input으로 사용한다. `mart.peak_feature_15min`이 있더라도 legacy/helper projection이며 canonical promotion 대상이 아니다. | leakage block |
