...
flowchart LR
    IN["review_input_plan_wording"]
    subgraph LG["langgraph_async_review"]
      STATE["state_contract_no_writes"]
      RET["retrieve_specs_context"]
      CHECK["qa_source_boundary_check"]
      DRAFT["review_note_recommendation"]
    end
    subgraph OUT["outputs"]
      NOTE["review_artifact"]
      APR["approval_recommendation"]
      CAVEAT["caveat_list"]
    end
    API["fastapi_job_registration"]
    OPS["ops_manual_approval"]
    IN --> STATE --> RET --> CHECK --> DRAFT
    DRAFT --> NOTE
    DRAFT --> APR --> OPS
    DRAFT --> CAVEAT --> API
    API --> IN
