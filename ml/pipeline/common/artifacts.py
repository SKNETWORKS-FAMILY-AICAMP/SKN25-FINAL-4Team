...
from ml.pipeline.common.config import ARTIFACTS_DIR


def _artifact_dir(meter_urn: str, horizon: int) -> Path:
...
def save_lstm_model(model, meter_urn: str, horizon: int, version: str) -> None:
...
def save_catboost(model, meter_urn: str, horizon: int) -> None:
...
def save_ridge(model, meter_urn: str, horizon: int) -> None:
...
def save_scalers(input_scaler, target_scaler, meter_urn: str, horizon: int) -> None:
...
def save_routing(routing: dict, meter_urn: str, horizon: int) -> None:
...
def save_bias_corrections(corrections_df, meter_urn: str, horizon: int) -> None:
...
def save_feature_columns(feature_columns: list[str], meter_urn: str, horizon: int) -> None:
...
def load_lightgbm(meter_urn: str, horizon: int, artifacts_dir) -> list:
    from lightgbm import Booster
    d = artifacts_dir / f"{horizon}h" / meter_urn
    models = []
    for step in range(1, horizon + 1):
        b = Booster(model_file=str(d / f"lightgbm_t_plus_{step}.txt"))
        models.append(b)
    return models
