...
실행:
  conda run -n skn25 python -m test6.pipeline.train --horizon 1
  conda run -n skn25 python -m test6.pipeline.train --horizon 3
"""
...
from ml.pipeline.common.db import build_engine, fetch_meter_frame
from ml.pipeline.common.preprocessing import build_windows, prepare_model_frame
from ml.pipeline.common.model import (
    RecurrentPredictor,
    add_anomaly_flags,
...
from ml.pipeline.common.catboost_model import fit_catboost, predict_catboost_scaled
from ml.pipeline.common.lightgbm_model import fit_lightgbm, predict_lightgbm_scaled, save_lightgbm
from ml.pipeline.common.ridge import fit_ridge, predict_ridge_scaled
from ml.pipeline.common.naive import predict_seasonal_naive
from ml.pipeline.common.router import v63_route_group
from ml.pipeline.common.ensemble import (
    build_prediction_df,
...
from ml.pipeline.common.plots import save_plots
...
from ml.pipeline.common.preprocessing import make_loader
from ml.pipeline.common.config import SMOOTH_L1_BETA
...
def pass1_train_meter(engine, spec: MeterSpec, horizon: int, args) -> dict:
    urn = spec.meter_urn
    ...
    for variant in LSTM_VARIANTS:
        ...
        save_lstm_model(model, urn, horizon, variant.version)
        model.eval()
        with torch.no_grad():
...
def _make_bundle(raw, spec, horizon, window_size, use_time_features):
    from ml.pipeline.common.preprocessing import prepare_model_frame, build_windows
...
def pass2_stage1(data: dict, args, v19_version: str, horizon: int) -> dict:
    ...
    try:
        ...
        save_lightgbm(lgb_models, urn, horizon, ARTIFACTS_DIR)
...
def pass2_finalize(pass1_data: dict, cand: dict, v63_vp: np.ndarray, v63_tp: np.ndarray,
    ...
    # 저장
    save_scalers(b.input_scaler, b.target_scaler, urn, horizon)
    save_feature_columns(b.feature_columns, urn, horizon)
    save_bias_corrections(corr, urn, horizon)
    save_routing({
        "meter_urn": urn, "horizon": horizon,
        "v19": v19_version, "v25": "v24" if use_v24 else v19_version,
        "v57": v57_ver, "v63": v63_ver, "ridge_alpha": alpha,
    ...
    out = ARTIFACTS_DIR / f"{horizon}h" / urn
