...
def run(
    ...
    try:
        ...
        print(f"[Ensemble] LSTM AE 스킵 (모델 없음 또는 오류): {e}")
        df3 = df2.copy()
        df3["anomaly_lstm"] = False
        df3["score_lstm"]   = 0.0
        has_lstm = False

    vote_cols = ["anomaly_stat", "anomaly_iso"] + (["anomaly_lstm"] if has_lstm else [])
    df3["vote_count"] = sum(df3[c].astype(int) for c in vote_cols)
